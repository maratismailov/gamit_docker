...
C 
      integer*4 MASK(32)  
C 
C     COMMON MSKCM CONTAINS THE SOLV2 STANDARD BIT MASK ARRAY 
C 
C     TO: VARIABLES IN MSKCM TO WHICH SBIT GIVES A VALUE: NONE
C 
C 
C     FROM: VARIABLES IN MSKCM FROM WHICH SBIT GETS A VALUE: MASK 
C     MASK(I), I=1,16, is a word in which only the I-th bit is set. 
C     MASK(1)=1, MASK(16) = 100000B (SIGN BIT)
C 
C 
C 
C 
...
C 
C     CALLED SUBROUTINES: 
C       UTILITIES IN SYSTEM: IAND(FUNCTION),IOR(FUNCTION),NOT(FUNCTION) 
C       SOLVE-2 UTILITIES (%%S2US): IGNPS(INT.FN) 
C 
C 3.  LOCAL VARIABLES 
C 
C     IVAL = INTERNAL VALUE OF IVALUE 
C 
C 
C END EQUIVALENCE 
C 
...
C# LAST COMPC'ED  820423:19:54 #
C
C TAH 87APR29: Replaced NARG with FTN77 routine PCOUNT
C
C JCP 81DEC16: INSTEAD OF NO-OP'ING WHEN IVAL IS NEITHER 0 NOR 1,
C              TREAT ANY NON-ZERO VALUE AS 1. I BELIEVE THIS MAKES
C              THE ROUTINE MORE CONVENIENT TO USE, AND THE RESULTS
C              BETTER DEFINED.
C              ALSO USE NARG INSTEAD OF IGNPS BECAUSE NARG IS IN
C              %%GSFC AND IGNPS IS NOT. THIS IS FOR THE CONVENIENCE
C              OF POTENTIAL NON-SOLV2 USERS.
CC LAST COMPC'ED  811216:14:09 #
C JCP 80 OCT 17: MADE IVALUE OPTIONAL AND DEFAULT TO 1. ONCE TOO MANY TIMES
C                I'VE HAD A CALLING  PROGRAM SCREW UP BECAUSE I FORGOT TO
C                CODE IN THE 1 AS THIRD ARGUMENT!
CMOD: JCP 79 Jul 27: Created.
C
C     PROGRAM STRUCTURE
C
C     1. GET # PARAMETERS IN CALLING SEQUENCE AND SET INTERNAL IVALUE APPROPRIATELY
C
      ival = ivalue
...
C
      ...
      if( ival.eq.1) then
          iarray(ia) = ior(iarray(ia),mask(ib))
      else
          iarray(ia) = iand(iarray(ia),not(mask(ib)))
      end if
C
C
C     3. IF IVALUE is 0, force the appropriate bit to 0.
C
C     IF$(IVAL.EQ.0) - SECOND TEST UNECESSARY 81DEC16.
C       ELSE IARRAY(IA) = IAND(IARRAY(IA),NOT(MASK(IB)))
C
C
