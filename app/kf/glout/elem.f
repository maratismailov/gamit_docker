...
*
*********1*********2*********3*********4*********5*********6*********7**
C     IMPLICIT DOUBLE PRECISION (A-H,O-Z),LOGICAL(L),Integer*4(i-n)
      SAVE
      LOGICAL LHYPER,LNEVEN,LKEP5,LNONS,LPARTS
      real*8 AEI(6),AEINPM(6),PKPX(6,6),XYZXYZ(6),
     &PH(3,6),CUXYZP(3)

      real*8 xi, yi, zi, xdoti, ydoti, zdoti, gm, a, e
      real*8 rincl, rnode, omega, rmean, x, y, z, xdot, ydot, zdot
      real*8 zero, one, two, twopi, degrad, scale, r

      real*8 rrdot, vsq, ainv, hx, hy, hz, hsini2,hsq, h, ome2
      real*8 esq, tempe, cosi, sini2, sini, hsini
      real*8 suprod, cuprod, resinf, recosf
      real*8 surecf, curecf, suresf, curesf

      real*8 true, ccc, swprod, cwprod, rtome2, aesine, aecose
      real*8 ecc, halfi, sinhaf, coshaf, sinn, cosn

      real*8 csprod, somega, comega, coson, sinon
      real*8 c1, ca1, ca2, ch1, ch2, ch3, eprod, ce1, ce2, cn1, cn2, cu5
      real*8 cosu, hcosu, hzcosu, sinu, hsinu, cu1, cu2, cu3, cu4
      real*8 adr, re, cosf, sinf

      real*8 c2, cf1, cf2, cf3, cf4, rda, cm1, cm2, ppfs, ppus, phps
      real*8 pups, scale2, c2j, c3j, c4j, c5j, c6j
      real*8 cf5, pfps



      integer*4 idrad, ihypsw, ieven, iparts, i,j, jflip
*
...
*  COMPUTE PARTIALS OF NONSINGULAR KEPLER ELEMENTS W/R TO CARTESIAN ELEM
  ...
  333 CONTINUE
      RETURN
 500  WRITE(6,10500)
      write(*,*) XI,YI,ZI,XDOTI,YDOTI,ZDOTI,AEI,IDRAD,PKPX,GM,
     1                IHYPSW
      STOP
