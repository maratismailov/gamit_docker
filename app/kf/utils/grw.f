...
c     The command is 'grw site yy ddd1 ddd2 hor ver'
c         where yy can be 2 or 4 digits, the dates are start and stop, and the last two
c         entires are horizontal and vertical noise in mm    
...
c     run
      ...
      integer iarg,iclarg,yr,doy1,doy2,mo1,mo2,day1,day2
     .      , hr1,hr2,mn1,mn2,ioerr
      ...
      if(iarg.eq.0) then
        write(*,'(a)') 
     .    'Runstring: grw [site] [yr] [doystart] [doyend] [Hor] [Vert]' 
     .   ,'  or (for all dates) '
     .   ,'           grw [site] all [Hor] [Vert]'
     ...
     .               ,iostat=ioerr)    
        ...
        if( ayr(1:3).eq.'all' ) then
          ...
          iarg = iclarg(3,adoy)    
          read(adoy,*) doy1      
          call monday(doy1,mo1,day1,yr)
          iarg = iclarg(4,adoy)   
          read(adoy,*) doy2
          call monday(doy2,mo2,day2,yr)
          iarg = iclarg(5,ahsig)   
          read(ahsig,*) hsig
          iarg = iclarg(6,avsig)  
          if( iarg.ne.0 ) then
        ...
        hr2 = 24
        mn2 = 0  
c** this no longer needed
c        if( site(5:5).eq.' ') site(5:5)='@'
        if( ayr(1:3).eq.'all' ) then
          write(1,'(1x,a,1x,a8,3f7.3)') 'sig_neu'
     .      , site,hsig/1000.,hsig/1000.,vsig/1000.
        else
          write(1,'(1x,a,1x,a8,3f7.3,2(1x,i5,4i3))') 
     .        'sig_neu'
     .      , site,hsig/1000.,hsig/1000.,vsig/1000.
     .      , yr,mo1,day1,hr1,mn1,yr,mo2,day2,hr2,mn2 
        endif
      endif
      stop
      end
