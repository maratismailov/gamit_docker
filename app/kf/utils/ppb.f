...
*
*     Runstring:
*     % ppb <sum_file> <min_number> <max_len> <min_len>
*
...
*   values(12)  - The eleven values needed from the summary
...
***** See if min length passed
      len_run = rcpar(4, line )
      if( len_run.gt.0 ) then
          read(line,*,iostat=ierr) min_len
          min_len = min_len
          if( ierr.ne.0 ) then
              call report_error('IOSTAT',ierr,'decod', line,
     .                        1, 'ppb/main')
...
*****         Now decode the rest of lines
              indx = 18
              read(line(18:),*,iostat=jerr) values(1)
              read(line(23:),*,iostat=jerr) (values(i),i=2,12)
...
*****             See if the data satisfies the conditions set.
     ...
     .                values(3).le. max_len .and.
     .                values(3).ge. min_len      ) then
                      num_ent = num_ent + 1
                      rname = line(1:8)
                      indx = 1
...
*                     Now get second site
     ...
     .                     iel, indx)
                      ...
                      length(num_ent) = values(3)*1000.
 
*                     Use rms about slope of duration > 0.2 year
*                     and number of values greater than 3
                      if( values(12).gt. 0.2d0 .and.
     .                    values(2).ge. 3 ) then
                          wrms(num_ent) = values(10)
                          chi(num_ent) = values(11)
                      else
                          wrms(num_ent) = values(6)
                          chi(num_ent) = values(7)
                      end if
...
*     paramters of the fit
      ...
      do i = 1, peak_soln
          ...
          do while ( .not.converged .and. iter.lt.30  )
              call clear_norm(norm_av, bvec_av, 1, norm_ch, bvec_ch,
     .                       norm_st, bvec_st,
     .                       num_sites )
              call get_ppb_est(i, converged,iter)
              iter = iter + 1
...
*            the site dependent averages.
     ...
     .       norm_ch(2*nav,2*nav), bvec_ch(2*nav),
...
CTITLE GET_PPB_EST
      ...
      subroutine get_ppb_est(is, converged, iter)
...
*   iter    - Iteration number (0 is first)
...
*     gn(2*max_sites)  - Gain array for fixing parameters
     ...
     .    gn(2*max_sites), hs_sum, hc_sum

      integer*4 hs_tot, hc_tot 
...
*                                         ! Yes it is, so increent
          if( num_soln(i).eq.is) then
              if( iter.eq.0 ) then
                  hs_sum = hs_sum + wrms(i)*num_est(i)
                  hs_tot = hs_tot + num_est(i)
                  hc_sum = hc_sum + chi(i)*num_est(i)
                  hc_tot = hc_tot + num_est(i)
              end if
              call bl_part( length(i), dbdh, dbdu, Re )
 
              num_used(is) = num_used(is) + 1
****          Now increment the average normal equatiosn
     ...
     .                hs_apr_av(is), us_apr_av(is), 1)
              ...
              call inc_norm( 1, 1, chi(i), num_est(i), dbdh, dbdu,
     .                bvec_ch, norm_ch, hc_apr_av(is), uc_apr_av(is),
     .                hc_apr_av(is), uc_apr_av(is), 1)

*****         Now increment site dependend values
C             call inc_norm( site(1,i), site(2,i), wrms(i), num_est(i),
C    .                dbdh, dbdu, bvec_st, norm_st,
C    .                hs_apr_st(is,site(1,i)), us_apr_st(is,site(1,i)),
C    .                hs_apr_st(is,site(2,i)), us_apr_st(is,site(2,i)),
C    .                num_sites)
...
*     try to a stabilize the system if we have negative values
      ...
      call fix_neg( norm_av, bvec_av, 2, us_av(is), 2, 0.0d0,cc,gn )
 
      if( abs(bvec_av(1)).gt. 0.01d-6) converged = .false.
      if( abs(bvec_av(2)).gt. 0.01d-6) converged = .false.
C     if( norm_av(2,2).gt.0 ) then
C         hs_con(is) = hs_av(is) - norm_av(1,2)*us_av(is)/
C    .                         sqrt(norm_av(2,2))*sqrt(norm_av(1,1))
C     else
C         hs_con(is) = hs_av(is)
C     end if
      if( iter.eq.0 .and.hs_tot.gt.0 ) then
          hs_con(is) = (hs_sum/hs_tot) 
          hc_con(is) = (hc_sum/hc_tot) 
      end if
      hs_apr_av(is)  = hs_av(is)
      hs_apr_con(is) = hs_con(is)
      us_apr_av(is)  = us_av(is)

****  Now solve the system of equations.  Do the Chi values .
      call invert_vis( norm_ch, bvec_ch, scale, pivot, 2, 2,1)
 
***** Get updated estimated and 
*     try to a stabilize the system if we have negative values

      hc_av(is) = hc_apr_av(is) + bvec_ch(1)
      call fix_neg( norm_ch, bvec_ch, 2, hc_av(is), 1, 1.0d0,cc,gn )
      uc_av(is) = uc_apr_av(is) + bvec_ch(2)
      call fix_neg( norm_ch, bvec_ch, 2, uc_av(is), 2, 0.0d0,cc,gn )
 
      if( abs(bvec_av(1)).gt. 0.01d-6) converged = .false.
      if( abs(bvec_av(2)).gt. 0.01d-6) converged = .false.
C     if( norm_ch(2,2).gt.0 ) then
C        hc_con(is) = hc_av(is) - norm_ch(1,2)*uc_av(is)/
C    .                         sqrt(norm_ch(2,2))*sqrt(norm_ch(1,1))
C     else
C        hc_con(is) = hc_av(is)
C     end if
      
C      write(*,*) 'hc_con',  hc_con(is), hc_av(is),
C     .            norm_ch(1,2), uc_av(is), norm_ch(2,2), norm_ch(1,1)
     
      hc_apr_av(is)  = hc_av(is)
      hc_apr_con(is) = hc_con(is)
      uc_apr_av(is)  = uc_av(is)

 
***** Now do the site dependent values
C     call invert_vis( norm_st, bvec_st, scale, pivot, 2*num_sites,
C    .                2*num_sites,1 )
...
C         hs_st(is,i) = hs_apr_st(is,i) + bvec_st(2*i-1)
C         call fix_neg( norm_st, bvec_st, 2*num_sites, hs_st(is,i),
C    .                  2*i-1, hs_av(is),cc,gn )
C         us_st(is,i) = us_apr_st(is,i) + bvec_st(2*i)
C         call fix_neg( norm_st, bvec_st, 2*num_sites, us_st(is,i),
C    .                  2*i,   us_av(is),cc,gn )
C
C     end do
...
***** First output average values
 ...
 100  format('* For solution number ',i3,' after ',i3,' iterations',/,
     ...
     .       '       CHI FITS ',/,
     .       '*   TYPE           a          PPB ',
     .       '     Average         a       PPB     Average   Iter',/,
     .       '*                 (mm)                (mm) ')

      end if 
      write(*,150) comps(is), 
     .            r2*hs_av(is), r2*us_av(is)/(2*Re)* 1.d6,
     .            hs_con(is), 
     .            r2*hc_av(is), r2*uc_av(is)/(2*Re)* 1.d6,
     .            hc_con(is), num_used(is), iter 

 150  format('*',2x,a4,3x,F10.2,3x,f8.3,3x,f10.2, 3x,
     .           F10.2,3x,f10.2,3x,f10.2,3x,2i4 )
 
 
****  Loop over sites
C     do i = 1, num_sites
C         write(*,150) site_names(i), hs_st(is,i), us_st(is,i),
C    .            us_st(is,i)/(2*Re)* 1.d6
C     end do
...
*   wrms_st - expected wmrs from site dependent results. (not used)

*   comps(4)  - Components
 
      real*8 dbdh, dbdu, wrms_av,  chi_av
...
*         Get the average and site dependent wrms expected
          ...
          wrms_av = sqrt((hs_av(ns)*dbdh)**2 +
     .                   2*(us_av(ns)*dbdu)**2 )
c          wrms_st = sqrt((hs_st(ns,site(1,i))*dbdh)**2 +
c     .                    (hs_st(ns,site(2,i))*dbdh)**2 +
c     .                    (us_st(ns,site(1,i))*dbdu)**2 +
c     .                    (us_st(ns,site(2,i))*dbdu)**2 )
 
          chi_av = sqrt((hc_av(ns)*dbdh)**2 +
     .                  2*(uc_av(ns)*dbdu)**2 )
     
*****     Now write out the resulys
          write(*,150) length(i)/1000.d0, wrms(i), chi(i), num_est(i),
     .                wrms_av, chi_av, num_soln(i), comps(num_soln(i)),
     .                site_names(site(1,i)), site_names(site(2,i))
 150      format( F10.3, F7.2,1x,f6.3,1x,i4,1x,2(F7.2,1x), I3,1x,a4,1x,
     .            a8,1x,a8)
      end do
