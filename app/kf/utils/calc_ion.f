...
*     J.L. Davis                   2:42 PM  THU., 16  APR., 1987
 
*     Program to calculate the ionospheric contribution for the
*     observations in a given KalObs file.  Proper runstring is
*
...
*       obs_code        - See <obs_code> above
 
      character*10 obs_code
...
***** Recover the runstrings
      ...
      len_obs_code = rcpar(3,obs_code)
 
***** Check that all three runstrings were given.  If not, Print help file
*         and abort
      if (len_KalObs_s * len_KalObs_x * len_obs_code .eq. 0)
     .    call proper_runstring(help_file,1,1)
 
***** Set flags corresponding to observables for which the ion contibution
*         is to be calculated
      call set_ion_type(obs_code)
 
***** Stop if nothing was asked for
      if (.not. (calc_ion_cont(1) .or.
     .           calc_ion_cont(2) .or.
     .           calc_ion_cont(3))    ) then
 
*****     That's all
          stop ' Abort:  No obs_code chosen'
 
      end if
 
***** Calculate the ionospheric delays
      call calc_ion_delays
 
      end
 
CTITLE SET_ION_TYPE
 
      subroutine set_ion_type(obs_code)
 
*     J.L. Davis                   3:55 PM  THU., 16  APR., 1987
 
*     Routine to set the calc_ion_cont flags in the header common
...
*       obs_code                    - <obs_code> passed from user
...
*       name_type(3)                - Full names
 
      character*20 name_type(3)
 
      data type / 'G', 'P', 'R' /
      data name_type / 'group delays',
     .                 'phase delays',
     .                 'phase delay rates' /
 
***** Initialize the calc_ion_cont flags
      do i = 1, 3
 
          calc_ion_cont(i) = .false.
 
      end do
 
***** Change <obs_code> to upper case
      call CaseFold(obs_code)
 
***** Loop over characters of obs_code
      do i = 1, TrimLen(obs_code)
 
*****     Initialize found flag for this character
          found = .false.
 
*****     Loop over observable type
          do j = 1, 3
 
*****         Check against this type
              if (obs_code(i:i) .eq. type(j)) then
 
*****             Write a message
                  if (.not. calc_ion_cont(j))
     .                write(*,100) name_type(j)(1:TrimLen(name_type(j)))
  100             format(' Ionospheric contribution will be calculated',
     .                   ' for ',A,'.')
 
*****             We will use this type
                  calc_ion_cont(j) = .true.
...
*****     If no type was found for this letter, tell user
          if (.not. found) write(*,200) obs_code(i:i)
  200     format(' No type corresponding to ',A,' was found.')
 
      end do
 
      end
 
CTITLE CALC_ION_DELAYS
 
      subroutine calc_ion_delays
 
*     J.L. Davis                   4:32 PM  THU., 16  APR., 1987
 
*     General driver routine for CALC_ION
...
***** Write results to both KalObs files
      do i = 1, 2
 
          call write_to_KalObs(i)
...
*             correction?
          if (calc_ion_cont(type) .and. ion_rec .gt. 0) then
...
*****     Set the bit if we calculated the ion delay
          if (calc_ion_cont(type)) call sbit(avail_baseline,bit,1)
