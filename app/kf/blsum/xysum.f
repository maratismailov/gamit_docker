...
c     and produce a XYZ      summary.  The summary will be output
...
*     Check for sort option
      ...
      if( len_sort.le.0 ) then
          call proper_runstring('xysum.hlp', 'xysum', 1)
      else
...
c.... write headr message
      write(*,50)
  50  format(//" XYSUM: Program to extract XYZ components from GLOBK",
     .   " solution",/,
     .         " ---------------------------------------------------",
     .   "---------",/)
c
...
c.... if we are in batch mode assign the file names from the runstring
      if( batch ) then
         apriori_file     = run_string(1)
         summary_file     = run_string(2)
         values_file      = run_string(3)
c
...
c....    open the summary file and the values files
     ...
     .       ' Error opening values file in batch mode'
         call check_soln_file(apriori_file, ierr,202,status_unk)
         if ( ierr.ne.0 ) stop
     .       ' Error opening apriori file in batch mode'
c
      end if
...
c
      write(200,100)
  100 format(/" XYSUM: GLOBK XYZ component summary: includes the",
     .   " following solutions") 
      write(202,105)
  105 format('* GLOBK Apriori coordinate and velocity file ',
     .    ' generated by XYSUM using:')
...
c.... Write out descriptive header
      write(200,140)
 140  format("  Stations    xyz  S#   #   mean length   sig   wrms",
     .  "   nrms     slope    sig    wrms  nrms   dur   mean    line #",
     . /,    "                                (m)       (m)   (mm)",
     .  "          (mm/yr)  (mm/yr)  (mm)        (yrs)  (yrs)")
      write(202,145)
 145  format('* Site          X             Y            Z     ',
     .       '     dX/dt   dY/dt   dZ/dt  Ref.        Position ',
     .       'Sigma         Velocity Sigma',/,
     .       '*              (m)           (m)          (m)    ',
     .       '    (m/yr)  (m/yr)  (m/yr)  Year     X (m)   Y (m)',
     .       '   Z (m)   dX/dy   dY/dt   dZ/dt (m/yr)')
...
* Saved values for outputting an .apr entry
* site_xyz(3,2) -- Position and velocity of site
* site_ref      -- Reference epoch (JD)
* site_sig(3,2) -- Velocity sigma for site

      real*8 site_xyz(3,2), site_ref, site_sig(3,2)
      integer*4 i, j
...
* enu(3) -- Denote NEU
      ...
      save  site_xyz, site_ref, site_sig
c
...
c
 ...
 100     format(a8,"-",a8,1x,i2,1x,i4,1x,
     .      f13.4,1x,f6.4,1x,f5.1,1x,f6.2,3x,
...
*     Now save and output information for generating an apriori file.
*     (Do only for the short output so that it will not be repeated).
      if( format.eq.'short' ) then
         if( enu_type.eq.1 ) site_ref = mean_year
*        Save values
         site_xyz(enu_type,1) = mean_out + 
     .                         (site_ref-mean_year)*slope_out/1000.d0
         site_xyz(enu_type,2) = slope_out/1000.d0
         site_sig(enu_type,1) = sig_mean_scaled
         site_sig(enu_type,2) = sig_slope_scaled/1000.d0
*        if this is the Z-component then output the line
         if( enu_type.eq.3 ) then
             write(unit+2,310) names(site_1), 
     .                  ((site_xyz(i,j),i=1,3),j=1,2),
     .                    site_ref,((site_sig(i,j),i=1,3),j=1,2)
 310         format(1x,a8,1x,3(f13.4,1x),3(f8.5,1x),f8.3,1x,6(f8.5,1x))
         endif
      end if
...
* MOD TAH 000123: See if too many sites
             if( num_site.ge.max_site ) then
                 call report_stat('FATAL','XYSUM','read_soln',
     .                ' ','Too many sites: Max allowed',max_site)
             endif
             call strip_name(buffer,names, num_site, iel,0)
         end if
...
*****    Now get rid of 'to '
         ...
         if( new_buffer(2:2).eq.'X' ) site(2) = 1
         if( new_buffer(2:2).eq.'Y' ) site(2) = 2
         if( new_buffer(2:2).eq.'Z' ) site(2) = 3
