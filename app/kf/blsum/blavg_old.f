...
c        data(4:7)   = epoch (julian date)
c        data(8:11)  = length (m)
c        data(12:15) = sigma (m)
c
...
c
      integer*4 irm(5), count, i, len_run, ierr, rcpar
c
...
*                              ! read rest of runstring
      do i = 1,3             
         len_run = rcpar(i, run_string(i))
c
         if( len_run.eq.0 ) then 
             call proper_runstring('blavg', 'blavg.hlp', 1)
         end if
c
      end do
c
...
*                                   ! get more file names
         do while ( more_files )
            ...
            write(icrt,'(" Enter the name of solution file #",i2,
     .         $)') num_files
c
...
c
c.... write the record
      write(unit,100) date,av_length, av_lsig, 
     .                av_slope, av_ssig, av_num
 
  100 format(i5,4i3,f16.4,1x,f10.4,1x,2(f10.4,1x),1x,i4)
c
      return
      end
c
...
*   new_gsoln*5     - Name GLOBK
*   new_gtime*24    - solution commenced with line
 
      character new_gsoln*5, new_gtime*24
...
c
     ...
     .     bas_to   / ' TO '     /
c
      data new_gsoln / 'GLOBK' /,
     .     new_gtime / 'SOLUTION COMMENCED WITH:' /
...
*                                 ! Solution found
            if( iel.eq.47 ) then
               ...
               if( num_soln.eq.0 .or. err.ne.0 ) then
                  write(*,'(" Solution number error in ",a,/,
     .                    " iel, indx, num_soln, err = ",4i8)')
...
C              write(loglu(id),*) base_data(1), base_data(2)
c
...
c
               call save_ema(data(1,num_ent), data(4,num_ent),
     .           data(8,num_ent),data(12,num_ent))
c
c....       Thats all
*                         ! baseline found
            end if
...
*                         Since we foumd something
     ...
     .                                      av_lsig)
                     ...
                     call acc_av(data(4,m), data(8,m), data(12,m))
                  end if
               end do
c
...
c
c.... write the record
      write(unit,100) date,length,sigma
  100 format(i5,4i3,f15.4,1x,f10.4,1x,"    0.000   0.000 ")
c
      return
      end
c
...
c
 ...
 100     format(a8,"-",a8,1x,i2,1x,i4,1x,
     ...
     .      f7.1,1x,f7.1,1x,f5.1,1x,f5.2,1x,
     .      f5.1,1x,f6.1,1x,i5,"-",i5)
c
*                  ! do the long output
      else
c
