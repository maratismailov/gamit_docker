...
c
*                                                 ! maximum number of
      parameter ( max_runstring = max_files + 3 )
...
c Output devices
c --------------
c apriori_file  -- Name of file for apriori coordinates to be written.
...
c
      character*128 apriori_file, solution_file(max_files), 
     .              summary_file, values_file
...
c res_out   -- Set true with -RES otion which outputs residuals to
c              globk aprorio model.

      logical batch, sort, realsigma, res_out
...
* Times(max_ts), Res(max_ts), Rerr(max_ts) -- Single component 
...
c
     ...
     .    Times, Res, Rerr, 
     .    norm_eq, b_vec, stat, num_ts,
     .    line_st, line_en, realsigma, res_out,
     .    apriori_file
