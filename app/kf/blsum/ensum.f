...
c     and produce a ENU      summary.  The summary will be output
...
*     Check for sort option
      ...
      if( len_sort.le.0 ) then
          call proper_runstring('ensum.hlp', 'ensum', 1)
      else
...
* MOD TAH 070917: See if option for residual option passed
      len_run(1) = rcpar(j+1,run_string(1))
      call casefold(run_string(1))
      if( run_string(1)(1:4).eq.'-RES' ) then
          j = j + 1
          res_out = .true.
      else
          res_out = .false.
...
c.... write headr message
      write(*,50)
  50  format(//" ENSUM: Program to extract ENU components from GLOBK",
     .   " solution",/,
     .         " ---------------------------------------------------",
     .   "---------",/)
      if( realsigma ) then
         write(*,'(a)') 'Realistic Sigmas for rates being computed'
      end if
      if( res_out ) then
         write(*,'(a)') 'Residuals to globk apriori will be output'
      end if
c
c
...
* MOD TAH 070608: See if QOCA file
* QOCA version:  1.30 03/01/2006 
      if( header(soln_num)(3:6).eq.'QOCA' ) then
*         Read a QOCA time series file
          call read_qoca_file( soln_num, vdata, data )
          return
      end if
...
*                                 ! Solution found
            if( iel.eq.47 .and. buffer(1:1).eq.' ' ) then
               indx = iel + 10
               call read_line(buffer,indx,'I4',err,num_soln,cdummy)
 
               if( num_soln.eq.0 .or. err.ne.0 ) then
* MOD TAH 021027: Added a to format so that is it correct 
                  write(message,'(3a,4i8)') 'Solution number error in '
     .                 , buffer(1:80),' iel indx num_soln err '
     .                 , iel,indx,num_soln,err 
                  call report_stat('WARNING','ENSUM','read_soln', ' '
     .                            ,message,0)
...
*                                 ! new clock epoch -- get value
            if( iclk.gt.0 .and. buffer(1:1).eq.' ' ) then
...
* MOD TAH 070917: Moved code to find_enu when residual output allowed
*              so that coordinates will contain total value.
C              if ( coords(site(2),site(1)).eq.0.d0 .and.
C    .              vdata(3,num_ent).lt.100.d0 ) then
C                   coords(site(2),site(1)) = vdata(2,num_ent)
C              end if
c
c....       Thats all
*                         ! baseline found
...
c.... Write out number of entries
  ...
  300 format(" There are ",i8," baseline entries between ",i4," sites")
      if( match_sep.gt.0 )
     .write(*,310) (names(i),(coords(j,i),j=1,3),i=1,num_site)
  310 format('Initial positions of sites',/,(a8,2x,3(F14.4,1x)))
c
      return
      end
c
...
c....          Now get the site coordinate data
     ...
     .                         cdummy,3)
* MOD TAH 070917: See if we outputing residuals
      if( res_out ) then
         base_data(1) = values(2)
      else
     ...
     .              values(3).lt.1.d0 ) then
           coords(site(2),site(1)) = values(1)
      end if
...
*                     ! Force exit on next read
         ierr = -99
         call report_stat('FATAL','ENSUM','find_enu',
...
c
      write(200,100)
  100 format(/" ENSUM: GLOBK NEU component summary: includes the",
     .   " following solutions")
      if( realsigma ) write(200,'(a)') 'Realistic Sigmas for Rates'
c
      do i = 1, num_files
...
c.... Write out descriptive header
      write(200,140)
 140  format("  Stations    enu  S#   #     mean length   sig   wrms",
...
c
      write(unit,120) names(i),enu(j),k,
     .      coords(j,i)
 120  format(a,' to ',a,' Solution ',i2,' + ',F13.3,' m')
c
      write(unit,'(1x)')
c
      return
...
c
c.... write the record
* MOD TAH 060109: Increased format to F16.5 to allow for sites
*     near the south pole. (N < -10000000 meters)
      write(unit,100) date,length,sigma, residual, sigma
  100 format(i5,4i3,f16.5,1x,f10.5,1x,2f10.5)
...
*      mean_year = date(1) + date(2)/12.d0 + date(3)/365.25
      call jd_to_decyrs(mean_epoch,mean_year)
c
c.... Now do the output
      num_len = stat(4,1)

* MOD TAH 060109: Increased format to F15.4 to allow for sites
*     near the south pole. (N < -10000000 meters)
      if( format.eq.'short' ) then
c
         write(unit,100) names(site_1),enu(enu_type), soln, num_len,
 ...
 100     format(a8,"-",a8,1x,i2,1x,i4,1x,
     .      f15.4,1x,f6.4,1x,f5.1,1x,f6.2,3x,
     .      f8.2,1x,f8.2,1x,f5.1,1x,f6.2,1x,
...
c
 ...
 200     format(/,"Wmean ",f15.4," m +- ",f6.4," from ",i4," data.",
     .      " WRMS ",f5.1," mm, NRMS ",f6.2)
...
*                See if sigma too large
                 if( values(2).lt.1.d0 ) then 
                    base_data(1) = values(1)
                    base_data(2) = values(2)
                    if( base_data(2).eq.0 ) base_data(2) = 0.001
...
* sigtol        -- Tolerance on sigma for tie (0.01 horizontal, 0.03 vertical)

      real*8 stats(2), sep, wgh, sigtol

***** Now loop over sites see which are close enough
C     do i = 1, num_site
C       write(*,320) names(i), (coords(j,i),j=1,3)
C320    format(1x,a8,3F20.4)
C     end do 
...
****  Save the original number of entries
 ...
 120  format('Checking ',i4,' sites for separations < ',F9.2,' m')
      do i = 1, num_site-1
         do j = i+1, num_site
...
* MOD TAH 090203: If coordinates of a site are zero (meaning no
*                 valid data) then increase separation so that they
*                 will not be linked.
               if( coords(k,i).eq.0 .or. coords(k,j).eq.0 ) then
                   sep = sep + 100*match_sep
               endif
            end do

            if( sep.le.match_sep**2 ) then
c               write(*,150) names(j), names(i)
c150            format('Mapping site ',a8,' to ',a8)
...
c               write(*,180) ni, nj, na
c180            format('Found ',i4,' and ',3i4,' match, ',3i4,' total')
...
*                     Now create entries for mapped sites
                      if( stats(1).ne.0 .and. 
     .                    names(j)(5:6).ne.'_X') then
                          write(*,220) na(m),m, names(j), names(i), 
     .                             stats(1)
...
* MOD TAH 070916: If residuals are being output, allow append with no
*                 overlapping data
                   elseif ( res_out .and. names(j)(5:6).ne.'_X' ) then
                          write(*,230) na(m),m, names(j), names(i) 
 230                      format('Adding ',i4,' Entries for coord ',
     .                           i2,' from ',a,' to ',a,
     .                          ' with no overlapping data')
...
CTITLE READ_QOCA_FILE
 
      subroutine read_qoca_file( soln_num, vdata, data )
 
      implicit none

 
*     Routine to read a qoca file to extract information about the
*     baseline lengths.
...
c
      real*8 values(10) ! Values read from qoca line
...
*   iel, in     - Index positon temporary storgae
      ...
      integer*4 indx, ierr, jerr, iel, in
 
*   cdummy      - Dummy character string for multiread
 
      character*1 cdummy
      character*8 read_name
...
*           Get the site name from the buffer
            indx = 0
            call multiread(buffer,indx,'R8',jerr,values,cdummy,10)
         end if
         if( jerr.eq.0 .and. ierr.eq.0 ) then 
            call read_line(buffer,indx,'CH',jerr,values,read_name)
*1987.4057    -3.28    -4.77    11.41    11.77 -0.0089    -3.42    36.90  0.0578 -0.0091  VNDN_GPS
*
            read_name(6:8) = 'QOC'
*           See if have site
            indx = 0
            call get_cmd(read_name, names, num_site, iel, indx )
            if( iel.le.0 ) then
                num_site = num_site + 1
                names(num_site) = read_name
                iel = num_site
            end if

            site(1) = iel
 
            call decyrs_to_jd( values(1), sol_epoch )


*           Save by component: North
            site(2) = 1
            num_soln = 1
            base_data(1) = values(3)/1000.d0
            base_data(2) = values(5)/1000.d0
            num_ent = num_ent + 1
c
c.....      see if maximun number of entries exceeded
            if( num_ent.ge.max_ent-3 ) then
               write(*,200) num_ent
  200          format(/" Maximum number of entries ",i8,
     .            " has been exceeded")
*                           ! Force exit on next read
               ierr = -99
               call report_stat('FATAL','ENSUM','read_qoca',
     .              'Maximum number of entries exceeded. ',
     .              max_ent )
            end if
            call save_ema(data(1,num_ent), vdata(1,num_ent))
            coords(site(2),site(1)) = 0.d0

****        East
            site(2) = 2
            base_data(1) = values(2)/1000.d0
            base_data(2) = values(4)/1000.d0
            num_ent = num_ent + 1
            call save_ema(data(1,num_ent), vdata(1,num_ent))
            coords(site(2),site(1)) = 0.d0
...
*           Height
            site(2) = 3
            base_data(1) = values(7)/1000.d0
            base_data(2) = values(8)/1000.d0
            num_ent = num_ent + 1
            call save_ema(data(1,num_ent), vdata(1,num_ent))
            coords(site(2),site(1)) = 0.d0
...
***** Write out number of entries
      max_soln = 1
      write(*,300) num_ent, num_site, max_soln
  300 format(' There are ',i8,' site entries between ',
     .                     i3,' sites, Solns ',i3 )
*
      return
      end
