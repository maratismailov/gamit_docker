...
* MOD TAH 190624: Added wild_date option (N will stop name being incremented).
                  call wild_date( glorg_apr_file, wild_mjd, 'N' )
...
* MOD TAH 190624: Added wild_date option (N will stop name being incremented).
                  call wild_date( glr_sol_file, wild_mjd, 'N' )
...
* MOD TAH 190531: Implemented -PER option in file name.
...
* MOD TAH 190531 See if -PER added to apriori file name
      ind_no_per = index(glorg_apr_file,'-PER')
      if( ind_no_per.gt.0 ) then
           no_per = .true.
           ind_no_per = ind_no_per-1
      else
           ind_no_per = trimlen(glorg_apr_file)
           no_per = .false.
      end if

****  OPen the apriori file: remove -PER
      open(101, file= glorg_apr_file(1:ind_no_per), status='old',
     .          iostat=ierr)
...
* MOD TAH 190531: See if we are ignoring PERIODIC terms
...
****  See if we should fix or just report the difference in apriori
      ...
      OK = .true.
      if ( kbit(options,21) ) then   ! FIXA option
...
* MOD TAH 180203: Updated logic for when positions differ by more than
*         a meter.  Positions need small update to account for difference
*         in velocity.  Not previous applied because there was no 
*         position problem.  Not an issue if consistent APR file used.
...
*         Change the position reference site if OK (<1 meter difference)
          if( OK ) then  
             do k = 1, 3
     ...
     .                     gsite_names(i)
          else   ! Change position only by velocity change
             do k = 1, 3
                dapr = (apr_val_site(k,2,i)*dtn)-
     .                 (apr_val_site(k,2,j)*dt)
                ! apr_val_site(k,1,j) = apr_val_site(k,1,j)
                np = parn_site(k, 1,j)
                if( np.gt.0 )
     .              parm_change(np) = parm_change(np)+dapr
             end do
             write(iout,140) 'dVel offset and Velocity', gsite_names(j),
     .                     gsite_names(i)
          end if
