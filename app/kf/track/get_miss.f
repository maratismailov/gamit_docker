CTITLE GET_MISS
* MOD AZ 190305: Build a Module to store VMF grid readinsg
        module vmf_type_build
        ...
        type(vmf_grid) :: VMF1_grid_file

        end module vmf_type_build

      subroutine get_miss(buffer,itype,indx)
...
*   pn          - PRN number
      ...
      integer*4 err, indx, jndx, i, date(5), ivals(5), pn
...
****  See if we should subs for <day> or <week>
     ...
     .      3700 , 3800, 3900, 4000, 4100, 4200, 4300, 9900 ) label
c
...
c.... file name of the GPS satellite position and clock 
  200 continue
         call multiread(buffer,indx,'CH',err,vals,cvalue,2)
         nav_file = cvalue(1)
         call subhome(nav_file)

         nav_file_type = cvalue(2)
         call casefold(nav_file_type)
         if (nav_file_type(1:3).eq.'SP3') sp3_file = nav_file
* MOD TAH 061228: Added reading file at this time so that we know
*        dates for reading phase center files
         if( nav_file_type.eq.'SP3') then
            call read_sp3(sp3_file ) 
         else
            call read_nav
         end if
         call read_antmod_svs
         return        
       
c
c.... GPS SV Clolk file name of the GPS satellite broadcast
  300 continue
...
c.... Ionospheric delay statistics.  Four basic parameters are
*     specified:
*     (1) Maxiumum ionospheric delay jump
*     (2) Ion delay in parts-per-million
*     (3) Contribution of ionospheric delay in RMS calculation
*     (4) Height of ionosphere
*     (5) Ion correlation distance
  700 continue
*        Read each of the values to see which ones are passed
         call read_line(buffer,indx,'R4',err,vals,cvalue)
         if( err.eq.0 .and. vals(1).gt.0 ) max_ion_jmp = vals(1)
         call read_line(buffer,indx,'R4',err,vals,cvalue)
         if( err.eq.0 .and. vals(1).gt.0 ) ion_ppm = vals(1)*1.d-6
         call read_line(buffer,indx,'R4',err,vals,cvalue)
         if( err.eq.0 .and. vals(1).gt.0 ) ion_wght = vals(1)
         call read_line(buffer,indx,'R4',err,vals,cvalue)
         if( err.eq.0 .and. vals(1).gt.0 ) ion_ht = vals(1)*1.d3
         call read_line(buffer,indx,'R4',err,vals,cvalue)
         if( err.eq.0 .and. vals(1).gt.0 ) ion_tau = vals(1)*1.d3

*        Now compute the other quantities needed
         ion_corr = (ion_ppm*ion_tau/(vel_light/fR1))**2
...
*        See if we match site
     ...
     .                      'Max allowed is ', max_edits
             ...
             call read_line(buffer,indx,'I4',err,ivals,cvalue)
* MOD TAH Allow: -1 as way of deleting all data at this time.
...
c.... AMBIN_FILE : ambiguity data file name 
 1500 continue
         call read_line(buffer,indx,'CH',err,vals,ambin_file)
         call subhome(ambin_file)
         return        
c
...
*      max_tot_search -- Limit on size of sample to search
 ...
 1700 continue
          ...
          if( vals8(1).gt.0 ) num_amb_samp = vals8(1)
          if( vals8(2).gt.0 ) relrank_limit = vals8(2)
          if( vals8(3).gt.0 ) max_tot_search = vals8(3)
          return        

c.... GEOD_OUT : Root for the geodetic position output files
 1800 continue
         call read_line(buffer,indx,'CH',err,vals,posit_root)
         return        
...
c.... SEARCH_TYPE : Ambiquity search type
 2000 continue
         call read_line(buffer,indx,'CH',err,vals,search_type)
         call casefold(search_type)
...
*.... RMS_EDIT_TOL: Sets limit on editing tolerance as a multiplier of
*        the data noise
 2300    continue
         call read_line(buffer,indx,'R4',err,vals,cvalue)
         if( vals(1).gt.0 ) rms_edtol = vals(1)
         RETURN
...
c..... MWWL_JUMP: Tolerance on adding 
 2900  continue
          jndx = indx
          call read_line(buffer,jndx,'R8',err,mwwl_jmp, cvalue)
          return
...
c....  REF_NEU: Defines reference coordinate of NEU output
 3300  continue
          ...
          return
* MOD AZ 190305: Introduce new option - VMF
*                Modify label name from USE_GPTGMF
*                to ATM_MODELC
c.... ATM_MODELC: Use GPT/GMF or VMF
 3400 continue
          atm_mtt = .false.
          call read_line(buffer,indx,'CH',err,vals,cval)
          call casefold(cval)
          if ( cval(1:3).eq.'GMF' ) then
*         See if the reference relative humifity passed
              atm_gmf = .true.
              call read_line(buffer,indx,'R8',err,vals8,cvalue)
                if( err.eq.0 ) ref_rel_hum = vals8(1)
          elseif ( cval(1:3).eq.'VMF' ) then
              atm_vmf = .true.
          else
             write(*,3401)
3401         format('ERROR: VMF integration failed',
     .               'ATM_MODELC command')
             stop 'TRACK: ahha3 Commands out of order'
          endif
          return
  
c.... TIME_UNIT: Sets time unit for process noise
 3500 continue
 ...
 3510         format('ERROR: INTERVAL command must be used before',
     .               'TIME_UNIT command')
          ...
          elseif( cval(1:1).eq.'S' ) then
              time_unit = 'second'
              tu_to_ep = 1/usr_interval
          elseif( cval(1:1).eq.'M' ) then
              time_unit = 'minute'
              tu_to_ep = 60/usr_interval
          elseif( cval(1:1).eq.'H' ) then
              time_unit = 'hour'
              tu_to_ep = 3600/usr_interval
          elseif( cval(1:1).eq.'D' ) then
              time_unit = 'day'
              tu_to_ep = 86400/usr_interval
          else
...
c.... IONEX_file: Name of IOPNEX file
 3800 continue
         call read_line(buffer,indx,'CH',err,vals, ionex_file)
         call subhome(ionex_file)
         use_ionex = .true.
         call read_ionex
         return 

c.... IONLOS_file: Name of IONLOS file
 3900 continue
*        Set the site name and match to site number
         call get_cmd(buffer,site_NUC,num_site,ns,indx)
         if( ns.gt.0 ) then  
             call read_line(buffer,indx,'CH',err,vals, ionlos_file(ns))
             use_ionlos(ns) = .true.
             print *,'use_ionlos ',use_ionlos(ns),' Site ',ns
             use_ionex = .true.
             ion_known = .true.  ! File is read before processing so 
                                 ! mark as knowm
         endif 
         return 

c.... MIN_TOLS: Minimum sigmas and correlations to be applied 
 4000 continue
         call read_line(buffer,indx,'R4',err,vals, cvalue)
         if( vals(1).gt.0 .and. err.eq.0 ) min_lcsig = vals(1)
         call read_line(buffer,indx,'R4',err,vals, cvalue)
         if( vals(1).gt.0 .and. err.eq.0 ) wl_tau = vals(1)
         call read_line(buffer,indx,'R4',err,vals, cvalue)
         if( vals(1).gt.0 .and. err.eq.0 ) dynamic_tol = vals(1)

c.... RM_CSLIP <# MW> <#EX> <EX max> <MW max> <EX sig> <MW max>
c     Decode the remove cycle slip command
 4100 continue
         call read_line(buffer, indx,'I4',err,ival,cvalue)
         if( err.eq.0 .and. ival.gt.0 )  num_mwwl = ival
         if( num_mwwl.gt. max_wl ) then
             write(*,4110) num_mwwl, max_wl
 4110        format('RM_CSLIP: Num MW WL too large. Input ',I4,
     .              ' reset to max_wl ',i4)
             num_mwwl = max_wl
         endif
         call read_line(buffer, indx,'I4',err,ival,cvalue)
         if( err.eq.0 .and. ival.gt.0 )  num_exwl = ival
         if( num_exwl.gt.num_mwwl ) num_exwl = num_mwwl
         call read_line(buffer, indx,'R4',err,vals,cvalue)
         ! Relative Rank
         if( err.eq.0 .and. vals(1).gt.0 ) NLx_RR = vals(1)
         call read_line(buffer, indx,'R4',err,vals,cvalue)
         ! Additive value to minimum value
         if( err.eq.0 .and. vals(1).gt.0 ) NLx_addChi = vals(1)
         ! Maximum value, minimum chi**2 have have to be fixed.
         call read_line(buffer, indx,'R4',err,vals,cvalue)
         if( err.eq.0 .and. vals(1).gt.0 ) NLx_minChi  = vals(1)

         return 

c.... Set GNSS systems to process.  Default is 'G' only.           
 4200 continue
         call read_line(buffer,indx,'CH',err,vals, tr_gnss)
         return

c     USE_BLQ to apply ocean tide
 4300 continue
* MOD TAH 200519: OPtions added.  IERS2010 and NOETIDE 
*        See if options are passed
         use_blq = .true.
         do i = 1,3

            call GetWord( buffer, cval, indx)
            if( trimlen(cval).gt.0 ) then
                call casefold(cval)
                if( cval(1:3).eq.'NOE' ) noetide = .true.
                if( cval(1:3).eq.'NOB' ) use_blq = .false.
                if( cval(1:3).eq.'IER' ) etide_2010 = .true.
            else
                exit
            endif
         end do
