CTITLE READ_ANTMOD
      ...
      subroutine read_antmod(file)

      implicit none

*     Routine to read and save phase center models.for SITES only
*
      include 'track_com.h'
...
CTITLE READ_DCBS
      ...
      subroutine read_dcbs(file, obs_mjd )
...
****  Try to open file
      ...
      call report_error('IOSTAT',ierr,'open',file,0,'read_dcbs')
      if ( ierr.ne.0 ) RETURN
...
****  Now read the file depending on version
     ...
     .          trimlen(line).gt.0 ) then
                ...
                if( eindx.ge.1 .and. eindx.lt.10 ) then   ! Epoch line
     ...
     .                   'read_dcbs/dcb')
                    if( jerr.eq.0 ) then
                        if( pn.le.max_prn ) then
                            ...
                            call report_stat('warning','track',
     .                         'read_dcbs',line,
 ...
 220            format(5x,I3,1x,I3,2x,I4,1x,I3,1x,I2,1x,I2,
     .                             2x, I4,1x,I3,1x,I2,1x,I2,1x,F10.3)
                if( jerr.ne.0 )
     .          call report_stat('warning','track','read_dcbs',line,
     .                 'IOSTAT error',jerr)
