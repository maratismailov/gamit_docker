...
****  check epoch to see if JD
      fract = epoch - int(epoch)
*     See if epoch needs to be convert from JD to MJD
c      TJD: JD-2440000.5d0
c      (r)MJD: JD-2400000.0d0
      if( epoch.gt. 2000000.d0 ) epoch = epoch - 2400000.d0
      mjd   = int(epoch) 
...
c      call make_rot( 2, cf_xp(1)/3600.d0*pi/180.d0, xrot)
c      call make_rot( 1, cf_yp(1)/3600.d0*pi/180.d0, yrot)
      call make_rot( 2, 0.d0, xrot)
      call make_rot( 1, 0.d0, yrot)
...
*       mJd_0hr       - Julian date a zero hours UT
      ...
      real*8 mjd,  t_0hr, gstd, diurnv, cent, mjd_0hr
