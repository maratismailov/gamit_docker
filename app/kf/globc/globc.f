...
*     at the end of runs.
      ...
      integer*4 trimlen, jerr, fmppurge, i

*   crt_file    - String for name of unit 6

      character*4 crt_file

*   Saved file names: for out_sol option
      character*256 save_comf, save_solf
      
*   kbit        - Logical to test bit status
      logical kbit 
      logical bakgls  ! Set true if we are running glorg in back solution
                      ! and we are saving GLX files.

*     Initialize to blank some file names that might be used
      save_comf = ' '
      save_solf = ' '
...
* MOD TAH 190620: Added feature to save GLX files during back
*     solution.  Don't run glsave at this point if a back
*     solution (glb_bak_soln) and glorg invoked (glr_cmd_file) 
      bakgls = .false.
      if( glb_bak_soln .and. trimlen(glr_cmd_file).gt.0 ) 
     .                                            bakgls = .true.
* MDO TAH 190620: Also check on bakgls 
      if( trimlen(glb_out_file).gt.0 .and. .not.bakgls ) then
          call report_stat('status','globk','main',glb_out_file,
     .                     'Saving loose combined global file',0)
          call run_glsave
      end if
...
*         Now see if we want to save the global file from glorg
          if( trimlen(glr_sol_file).gt.0 .and.
     .        trimlen(glb_out_file).gt.0      ) then
              save_comf = glb_com_file
              save_solf = glb_sol_file
              glb_out_file(trimlen(glb_out_file)+1:) = '.CON'
              call report_stat('status','globk','main',glb_out_file,
     .                     'Saving loose combined global file',0)
              call run_glsave
          end if
...
* MOD TAH 050524: See if scracth files are to be deleted
      if( del_scratch ) then
         print *,'DELETING COM, SOL, SRT and MAKE_SVS FILES (del_scra)'
         jerr = 0
         if( trimlen(glb_com_file).gt.0 ) jerr = FmpPurge(glb_com_file)
         call report_error('IOSTAT',jerr,'delet',glb_com_file,0,
     .                     'globk/glb_com_file') 
         if( trimlen(save_comf).gt.0 ) then
            jerr = FmpPurge(save_comf)
            call report_error('IOSTAT',jerr,'delet',save_comf,0,
     .                        'globk/save_comf')
         endif 

         if( trimlen(glb_sol_file).gt.0 ) jerr = FmpPurge(glb_sol_file)
         call report_error('IOSTAT',jerr,'delet',glb_sol_file,0,
     .                     'globk/glb_sol_file') 
         if( trimlen(save_solf).gt.0 ) then
            jerr = FmpPurge(save_solf)
            call report_error('IOSTAT',jerr,'delet',save_solf,0,
     .                        'globk/save_solf')
         endif 
         if( trimlen(sort_file).gt.0 ) jerr = FmpPurge(sort_file)
         call report_error('IOSTAT',jerr,'delet',sort_file,0,
     .                     'globk/sort_file') 
         if( make_svs_file ) then
            jerr = FmpPurge(glb_svs_file)
            call report_error('IOSTAT',jerr,'delet',glb_svs_file,
     .                      0,'globk/glb_svs_file') 
         endif
