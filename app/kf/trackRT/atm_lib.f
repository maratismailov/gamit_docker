...
CTITLE GET_ATMRT
      ...
      subroutine get_atmRT( ns, mjd, dry_zen, wet_zen, lat, long, ht )
...
CTITLE REPORT_ATM_REFRT

      subroutine report_atm_refRT(unit)
 ...
 220  format('INTERPOLATED ZENITH DELAYS FROM ',a,/,
     .       'YYYY MM DD HR MIN ',20(3x,a4,1x))
      ...
      do i = 1,12
         ...
         call mjd_to_ymdhms(mjd, date,sectag )
         do ns = 1, num_site
             call get_atmRT( ns, mjd, dry_zen, wet_zen, lat(ns), 
     .                    long(ns), ht(ns) )
