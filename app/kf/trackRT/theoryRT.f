CTITLE THEORYRT
      ...
      subroutine theoryRT(trn_time, trn_sec, ns, pn, site_pos, svs_ear, 
...
*     trn_time, trn_sec, ns, pn and site_pos are input variables
...
***** Make sure it is OK
      if( pn.eq.0 ) then
         write(*,120) ep, ns
 120     format('*** NO PRN at Epoch ',i6,' Site ',i2)
         return
      end if

****  Get the satellite clock correction
      send_mjd = trn_time - svs_clk(pn)/86400.d0
      send_sec = trn_sec - svs_clk(pn)

      call eph_to_xyzRT( send_mjd, send_sec, pn, 'E', ep )

      if( opt(1:1).eq.'S' ) then
          range(1) = sqrt( (site_pos(1)-svs_xyz(1,pn))**2 +
     .                 (site_pos(2)-svs_xyz(2,pn))**2 +
     .                 (site_pos(3)-svs_xyz(3,pn))**2 )
          range(2) = range(1)

          svs_t  = trn_time
          site_t = trn_time + (range(1)/vel_light)/86400.d0  

          call earth_to_inert( site_t, site_pos, site_L1i, 'E','I')
          call earth_to_inert( svs_t, svs_xyz(1,pn),svs_inert,'E','I')

          range(1) = sqrt( (site_L1i(1)-svs_inert(1))**2 +
     .                 (site_L1i(2)-svs_inert(2))**2 +
     .                 (site_L1i(3)-svs_inert(3))**2 ) 
          range(2) = range(1)

          if( ep.ge.debug(1) .and. ep.le.debug(2) ) then
              write(*,140) ep, send_mjd, send_sec, pn, 
     .              site_pos, svs_xyz(:,pn)
 140          format('SRange EP MJD Sec PRN ',I6,F14.6,F14.6,1x,i2,
     .               ' Site SVS Pos ',3F14.3,1x,3F16.3)
          end if
...
* MOD TAH 100517: Check elevation angle to make sure not less than zero
 ...
 150      format('WARNING: Small elvation angle Ep ',i6,' Site ',a,
     .           ' PRN ',I2.2,' MJD ',F15.8,' Elev ',F6.2,' dg')
          elev = 0.1d0
      end if
...
****  Add in Earthtide correction
      jd_tide = send_mjd + 2400000.5d0
      call earth_tide( jd_tide, site_pos, dXYZ_tide )
* MOD 090116: Added option to turn off tides
      ...
      do i = 1,3
          site_L1e(i) = site_L1e(i)+dXYZ_tide(i)
          site_L2e(i) = site_L2e(i)+dXYZ_tide(i)
      end do         
...
*        Now get the zenith delay terms and map to the elevation angle
         if( .not.use_atm_ref ) then 
             ...
             call get_atmRT( ns, trn_time, dry_zen, wet_zen, 
     .                     lat, long, ht )
         end if
...
*     Now compute the atmospheric delay
      atm_range = (dry_zen+atm_offset(ns))*dry_map + wet_zen*wet_map
...
****  Correct the satellite for the phase center position
      call svs_cm_to_phsRT( svs_t, pn, svs_inert, svs_phs )
     ...
     .                 (site_L2i(3)-svs_phs(3))**2 ) + atm_range
      if( ep.ge.debug(1) .and. ep.le.debug(2) ) then
           write(*,160) ep, send_mjd, send_sec, pn, 
     .           site_L1i, svs_phs(:)
 160       format('SRFull EP MJD Sec PRN ',I6,F14.6,F14.6,1x,i2,
...
*     Compute the satellite nadir angle
      ...
      dsvsL1 = IntAntMod(num_svs_dph(1,pn), svs_dna(1,1,pn),
     .                   svs_dphs(1,1,1,pn), dnadir, az,
     .                   max_dna, 1, ob(2))
      dsvsL2 = IntAntMod(num_svs_dph(1,pn), svs_dna(1,1,pn),
     .                   svs_dphs(1,1,2,pn), dnadir, az,
     .                   max_dna, 1, ob(2))
...
cd     .          ' EP ',i6,' Nadir ',F6.2,' PCV ',2F9.4,' m')
      ...
      range(1) = range(1) + dsitL1 + dsvsL1
      range(2) = range(2) + dsitL2 + dsvsL2
...
*     from theory module here 
      phase(1) = range(1)*fL1/vel_light
      phase(2) = range(2)*fL2/vel_light

      dcbap(1) = 0   ! For debug output 
      dcbap(2) = 0
      if( rcv_type(ns)(1:1).eq.'C' ) then
          range(1) = range(1) - dcbs(pn)
          dcbap(1) = dcbs(pn)
      elseif( rcv_type(ns)(1:1).eq.'P' ) then
          range(1) = range(1) - dcbs(pn)
          range(2) = range(2) - dcbs(pn)
          dcbap(1) = dcbs(pn)
          dcbap(2) = dcbs(pn)
      end if
      ...
      if( ep.ge.debug(1) .and. ep.le.debug(2) ) then
         write(*,200)
 200     format(20('-1'),' THEORY OMC ',20('-1'))
 ...
 210     format('Site ',a4,' Position ',3F14.4,/,
     ...
     .         ' L2 NEU phase center ',3F8.4)
         write(*,220) ns, site_pos,
     .        (dXYZ_L1(i), i=1,3), (dXYZ_L2(i),i=1,3)
 220     format('Site ',i4,' Position ',3F14.4,/,
     .          'L1 XYZ phase center ',3F8.4,
     .         ' L2 XYZ phase center ',3F8.4)
         write(*,230) site_names(ns), range(1), site_L1i,
     .           site_names(ns), range(2), site_L2i
 230     format('Range+Inert coords ',a4,' L1 ', F18.4, 3F14.4,/,
     .          'Range+Inert coords ',a4,' L2 ', F18.4, 3F14.4)

         write(*,240) (dXYZ_tide(i),i=1,3)
 240     format('Earthtide dXYZ ',3F8.4)

         write(*,250) pn, rcv_type(ns),dcbap
 250     format('APPLIED DCB for PRN ',i2.2,' Type ',a1,
     .          ' P1/P2 ',2F8.3,' m')

         write(*,260) pn, (svs_phs(i)-svs_inert(i),i=1,3)
 260     format('SVS phase center PRN ',i2,' dXYZ ',3F8.4)
 ...
 300     format('PVC: Site ',i2,' PRN ',i2,' Elev, Nadir, AZ ',
     .          3F8.2,' Site L1/L2 ', 2F9.4,' SVS L1/2 ',2F9.4)

         write(*,320) ep, site_names(ns),pn, range, phase
 320     format('Final Range Ep ',i6,1x,a,1x,' G',i2.2,1x,
...
CTITLE SVS_CM_TO_PHSRT
 
      subroutine svs_cm_to_phsRT( mjd, pn, svs_non, svs_phs )
...
*   prn_blk(32) - Block numbers for each PRN (upto PRN32)
C      integer*4 prn_blk(32)
...
C     if( abs(tdt-old_tdt).gt.0.02 ) then
C         write(*,100) (mjd-int(mjd))*86400.d0, shat,
C    .                 xhat, yhat, ehat, 
C    .                (mjd-int(mjd))*86400.d0, svs_non

C100      format('SUN: ',f10.4,3F10.6,' Xhat ',3F7.4,' Yhat ',3F7.4,
C    .           ' Zhat ',3F7.4,/,'SUN SVS: ',f10.4,3F16.4)
C         old_tdt = tdt
C     end if

*     See which type of sateliitet (valid Jan 1994).
C     if( prn_blk(pn).eq.1 ) then
C         sv_cm(1) = 0.2100d0
C         sv_cm(2) = 0.d0
C         sv_cm(3) = 0.8540d0
C     else if( prn_blk(pn).eq.2. .or.prn_blk(pn).eq.3 ) then
C         sv_cm(1) = 0.279d0
C         sv_cm(2) = 0.d0
C         sv_cm(3) = 1.023d0
C     else if( prn_blk(pn).eq.4.) then
C         sv_cm(1) = 0.000d0
C         sv_cm(2) = 0.000d0
C         sv_cm(3) = 0.000d0 
* MOD TAH 051202: Added Block 5 type and fatal if block not found
C     else if( prn_blk(pn).eq.5.) then
C         sv_cm(1) = 0.000d0
C         sv_cm(2) = 0.000d0
C         sv_cm(3) = 0.000d0
* MOD TAH 061143: Added Block 6 fot IIRM (Relative model values)
C     else if( prn_blk(pn).eq.6.) then
C         sv_cm(1) = 0.000d0
C         sv_cm(2) = 0.000d0
C         sv_cm(3) = 0.000d0
C     else 
C         call report_stat('FATAL','TRACK','therory',' ',
C    .        'Unknown satellite block number', prn_blk(pn)) 
