...
* MOD TAH 050710: Added reading satellite phase center positions
...
*                         Get the time from the line.
 ...
 215                          format(27x,i2,1x,i3,1x,i5)
                          ...
                          ne = ne + 1
* MOD TAH 191004: Take csec to nearest 10 minuute to handle GYPSY-X which
*                 tags everything 150 seconds before hour.
                          csec = nint(csec/600.d0)*600
                          call yds_to_jd( cyr, cdoy, csec, epvals(ne))
                      end if

                      if( line(8:10).eq.'VEL'  ) np = np+1

                      if( line(8:11).eq.'SATA' ) np = np + 1

*                     Check translation and scale
                      if( line(8:11).eq.'TRAN' ) np = np +1
C                     if( line(9:11).eq.'GC ' .and. 
C    .                    line(23:26).eq.'----') np = np+1
* MOD TAH 171002" Removed test for ----. In IGS SNX it is '   1'
                      if( line(9:11).eq.'GC ' ) np = np+1
C   729 XGC    ---- -- ---- 03:344:86392 m    2 .14661719634718505E-2 .1776647E-2
...
*          Check the occupancy value
           ...
           if( ierr.ne.0 ) then
               write(*,250) chocc, trim(line)
 250           format('**ERROR Decoding chocc in ADD_SITE_NAME ',
     .                'CHOCC ',a4,' Line ',a) 
               occ = 1  
           endif                 
      end if
