...
************************************************************************
     ...
     +               width, u1, u2, u3, scale, makegrid, numsta, 
     +               numflts, xmin, xinc, xnum, ymin, yinc, ynum, 
     +               slong, slat, sname, origlong, origlat,infile,
     +               outfile)
      include 'param.displace.h'
      character*256 line
...
C     Write the fault characteristics to the file.
     ...
     +     'dip','length',' width', 'u1', 'u2', 'u3', 'scale'
      do 460 m = 1, numflts
...
*     Get the name of the input site and setup file.
      ...
      if( len_run.le.0 ) then
        call proper_runstring('displace3.hlp', 'displace', 1)
      end if
...
************************************************************************
     ...
     +               width, u1, u2, u3, scale, makegrid, numsta, 
     +               numflts,xmin, xinc, xnum, ymin, yinc, ynum, slong, 
     +               slat, sname, infile)
...
C     Write out locations and the displacement heights
 ...
 604  format(4A10)
      if (makegrid.eq.0) then
         do 640 n = 1,numsta
            write(106,614) slong(n), slat(n), 0, displu(n),
     +           0,0,0
 640     continue
      else
         do 639 n = 1,numsta
