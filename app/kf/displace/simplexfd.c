...
int  rel_ref[8] ; /* index #'s of reference site 
                    MOD TAH 170408 Increased from 3 to 8 */
int  ndata_actual ;
...
/*  First open the input and output files  */
 ...
 if( in_fp == NULL ) {
    printf("Error opening input file %s Exit\n",argv[1]);
    exit(6);
 }
 out_fp = fopen(argv[2],"w") ;
 if( out_fp == NULL ) {
    printf("Error opening output file %s Exit\n",argv[2]);
    exit(6);
 }

 strcpy(outfault,argv[1]);
/* strtok(outfault,".-_"); 
 MOD TAH 170409: Use just . as delimiter */
 strtok(outfault,".");
        ...
        if( print_flag )
           {
            if(obs[k].type == 8)
              ...
              fprintf(out_fp, 
                "%3d\t%10.4f\t%10.4f\t%10.4g\t%10.4g\t%10.4g\t%10.4g %1d\n",
                (obs+k)->type , (obs+k)->xloc , (obs+k)->yloc ,
                observ , calc , (obs+k)->resid, uncert, print_flag );
           }
...
{   
 int i;
 double moment, Mw  ; /* Moment dyne-cm and Mw */
 double totslip ;     /* Total slip in mm */

 fault_fp = fopen(outfault,"w") ;
 if( fault_fp == NULL ) {
    printf("Error opening output fault file %s\n",outfault);
 }
 printf("Opened fault file %s \n",outfault) ;
...
/* Now compute Moment and Mw ; Factor mu 3e10 Pascal=3e17 dyne/cm^2 km^2=m^2 1e6 */
     totslip = sqrt((the_flt+i)->u1*(the_flt+i)->u1+(the_flt+i)->u2*(the_flt+i)->u2);
     moment = 3.0e23*(the_flt+i)->wth*(the_flt+i)->len*totslip*1e-3;  /* Convert slip mm->m */
     Mw = 2*log10(moment)/3 -10.7 ;
     fprintf(out_fp,"Moment %10.3e dyne-cm ; Mw %6.2f Total Slip %10.2f mm \n",moment, Mw, totslip);
     ...
     switch(ityp)
           {
            case 1:
               ...
               fprintf(out_fp, "error longitude = %f\n", temp);
               break ;

            case 2:
               temp =  stddev[i] * typical_dist ;
               fprintf(out_fp, "error latitude = %f\n", temp);
               break ;

            case 3:
               temp =  stddev[i] * (360.0/pi)/(1+average[i]*average[i]) ;
               fprintf(out_fp, "error strike = %f\n", temp);
               break ;

            case 4:
               temp =  stddev[i] * (360.0/pi)/(1+average[i]*average[i]);
               fprintf(out_fp, "error dip = %f\n", temp);
               break ;

            case 5:
               temp =  exp(average[i]) * typical_dist * stddev[i] ;
               fprintf(out_fp, "error depth = %f\n", temp);
               break ;

            case 6:
               temp =  exp(average[i]) * typical_dist * stddev[i] ;
               fprintf(out_fp, "error width = %f\n", temp);
               break ;

            case 7:
               temp =  exp(average[i]) * typical_dist * stddev[i] ;
               fprintf(out_fp, "error length = %f\n", temp);
               break ;

            case 8:
               temp =  stddev[i] * typical_slip ;
               fprintf(out_fp, "error strike-slip = %f\n", temp);
               break ;

            case 9:
               temp =  stddev[i] * typical_slip ;
               fprintf(out_fp, "error dip-slip = %f\n", temp);
               break ;
           }
    }
}
...
void OutPutCor2(double *correlation)
{
 ...
 for (i=0;i<ndim;i++)
	{	
	 for (j=0;j<=i;j++)
		fprintf(out_fp, "%12.8f     ", correlation[IX(j,i)]);
	 fprintf(out_fp, "\n");
	}
}
