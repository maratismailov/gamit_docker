CTITLE    ...................................................................
      ...
      subroutine dyaxis_mn
c
      implicit none

c     Routine to draw an y axis at x min
c
c Include files
c -------------
*                          ! the parameter file
...
c
c.... Get the spacings for a line feed
      lfc(1) =  0.0
      lfc(2) = -1.0
 
*     Set orientation and justification of plot
      call scj(1.0, 0.0, 1.0, 0.5)
...
c
c.... Draw the axis
      call draw_axis(scale_size(1),scale_size(3), scale_size(1),
     .   scale_size(4))
c
...
*           see if we are at a major tic.  If so increase length.
     ...
     .          mod((tic_num-0),label_mult).eq.0 ) then
            ...
            call draw_tic(scale_size(1), tic, majtl*tic_len, 0.,
     .                                   -(majtl+1.2)*tic_len, 0.)
            tic_num = tic_num + 1
c
...
*                                ! Use the current orientation of the labels
         call comlf( lfc, lf )
         call label_axis( scale_size(1),
     .      (scale_size(3)+scale_size(4))/2.0, 1, label)
