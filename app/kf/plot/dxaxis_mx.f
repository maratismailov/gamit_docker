CTITLE    ..................................................................
      ...
      subroutine dxaxis_mx
c
      implicit none

c     Routine to draw an xaxis at y max
c
c Include files
c -------------
*                          ! the parameter file
...
*     Set orientation and justification of plot
      call scj(1.0, 0.0,  0.5, 0.0)
 
*     Now rotate into correct frame
      call rotor
c
c.... Get the format for the labels if this is type 1
*                                 ! get format
* MOD TAH 200331: Added field type 3
...
c
c.... Draw the axis
      call draw_axis(scale_size(1),scale_size(4), scale_size(2),
     .   scale_size(4))
c
...
*           see if we are at a major tic.  If so increase length.
     ...
     .          mod((tic_num-0),label_mult).eq.0 ) then
            ...
            call draw_tic(tic,scale_size(4), 0.,-majtl*tic_len,
     .                        0., (majtl+1.2)*tic_len)
            tic_num = tic_num + 1
c
