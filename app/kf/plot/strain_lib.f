...
c                            : positive strain is extension!
      real*4 edot11,edot12,edot22,eps1snr,eps2snr
...
c              go to opposite side and come back
      ...
      subroutine make_clock( spin,spinsig,sscale, dxp, dyp, nump)
 
c     Make a clock diagram to show rotation rates
c     Kurt Feigl 9100523
 
c     INPUT VARIABLES:
c        sscale               : scaling factor for size of cloverleaf
c        spin, spinsig        : CW rotation rate and sigma in rad/yr
c     OUTPUT VARIABLES:
c       dxp, dyp                : arrays of X and Y 
c       nump                  : number of points
       real*4 sscale,spin,spinsig
       real*4 dxp(*), dyp(*)
       integer nump,nstep,i

c     LOCAL VARIABLES:
c       x0, y0               : UV coordinates of origin

      real*4 th,x0,y0,x1,y1,spin10,sig10,th0
      
c     How far would we spin in 10 million years
      spin10 = 10.e6 * spin
      sig10  = 10.e6 * spinsig
...
c     print *,'MAKE_CLOCK: ',
c     .spin10 * 45/datan(1.0d0),
c     .sig10  * 45/datan(1.0d0)
      
      nstep = 100

c     make a wedge as wide as the rotation in 10 Myr,
c     with a line for every 0.02 microrad/yr
      do i = 0,nstep
         th = i * spin10/nstep
         x1 = x0 + sin(th) * sscale
         y1 = y0 + cos(th) * sscale
...
c        draw a line to the middle 
         if (abs(th-th0) .ge. 0.2) then
c           go to zero and come back
            nump = nump + 1
            dxp(nump) = x0
            dyp(nump) = y0
...
c     make a dense wedge to show the uncertainty
      nstep = 30
      do i = -nstep,nstep
         th = spin10 + i * sig10/nstep
         x1 = x0 + sin(th) * sscale * 0.67
         y1 = y0 + cos(th) * sscale * 0.67
...
CTITLE MAKE_MANDALA
      ...
      subroutine make_mandala( eps1,eps2,theta,thetasig,eps1sig,eps2sig,
     .                        sscale, dxp, dyp, nump)
 
c     Make a strain mandala
c     Kurt Feigl 910504
 
c     INPUT VARIABLES:
c        sscale               : scaling factor for size of mandala
c        theta,thetasig       : azimuth of more compressive eigenvector (deg)
c        eps1,eps2            : eigenvalues of strain rate (1/yr)
c        eps1sig,eps2sig      : uncertainties of eigenvalues of strain rate (1/yr)
c     OUTPUT VARIABLES:
c       dxp, dyp                : arrays of X and Y 
c       nump                  : number of points
       real*4 sscale,theta,eps1sig,eps2sig,eps1,eps2,thetasig
       real*4 dxp(*), dyp(*)

       integer nump,istep(4),i,j,k
...
c                            : 1-axis points E, 2-axis points N
c                            : positive strain is extension!
      real*4 eps1snr,eps2snr
      real*4 degrad,th,eps,x0,y0,x1,x2,y1,y2,th0
c     step size in degrees
      integer nstep,nth0,nrad
...
c     write (6,*) 'MAKE_MANDALA: sscale = ',sscale
...
c     loop once for eps1 and once for eps2 
      do j = 1,4
         nstep = nint(thetasig) 
         nump = 0
         ...
         th0 = theta - (j-1) * 90.
         nth0 = nint(th0)
         do i = nth0-nstep,nth0+nstep                                   
            th = i * degrad           

c           remember, theta corresponds to eps2, not eps1
...
c           extensional quadrant                               
            if ((eps.gt.0.d0 .and. istep(j).ne.0 .and.
     .          mod(i,5*istep(j)).eq.0) .or. 
     .          (i .eq. -nstep .or. i .eq. nstep)) then
c               go to zero and come back                     
                nump = nump + 1                              
                dxp(nump) = x0                               
                dyp(nump) = y0                               
...
c        compressional quadrant: draw stripes
         if (eps .lt. 0.0d0 .and. istep(j) .ne. 0) then
            do i = -nstep,nstep,2*nstep
              nrad = istep(j) * 3
              do k = 1,nrad
c                go to opposite side and come back
                 x2 = x0 + eps*sin(th0-(th-th0))*sscale*k/nrad
                 y2 = y0 + eps*cos(th0-(th-th0))*sscale*k/nrad
                 nump = nump + 1                                    
                 dxp(nump) = x2
                 dyp(nump) = y2                                     
                 nump = nump + 1                                    
...
*               - coordinates
*   eu, ev      - End of the arrow  (not used)
 
      real*4 su, sv, en_to_mm(2,2), arrow_length,  value
...
c           Make the cloverleaf in relative coordinates
     ...
     .         sscale, dxp, dyp, nump )
         else if (istrain .eq. 1) then
c           Make the mandala in relative coordinates
            call make_mandala(strain(1,i),strain(2,i),strain(4,i),
     .         strainsig(4,i),strainsig(1,i),strainsig(2,i),
     .         sscale, dxp, dyp, nump )
         else if (istrain .eq. 2) then
            call  make_clock(strain(3,i),strainsig(3,i),
     .         sscale, dxp, dyp, nump)

*        See if conventional type strain errors.
         else if ( istrain .eq.3 ) then
            call make_strain_arrow(strain(1,i),strain(2,i),strain(4,i),
     .            strainsig(1,i),strainsig(2,i),
     .            arrow_length,sscale, dxp, dyp, nump )
         else
            print *,'SDRAW: invalid type: ',istrain
            return
         endif
...
c        Now draw the cloverleaf, mandala, or clock
...
*   id          - Dummy for readline
      ...
      integer*4 i,j, idata(int_recl), largest_field, indx,
     .    ierr
...
*   dxp(10), dyp(10)  - Five points making at the arrow.
      ...
      real*4 dx, dy, arrow_len, dxp(*), dyp(*)
