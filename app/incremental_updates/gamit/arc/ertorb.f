...
c     M. Floyd 2020/01/13: Added "BEIDOU-3I" SV body type to block
c                          defining d0 for "BEIDOU-2I" and "BEIDOU-2G"
c
...
c          kkk = eqn group  ( 0 for motion, 1 -> 9 for partials
     ...
     .     ,fomeg0,asat,anomin,wper0,eccen,fincl
c*     .     , k2
     .     ,decyrs 

* MOD TAH 190709: Added uarg and associate functions so that 
*     u can be used with BERNE model and u-u0 with ECOM1 and ECOMC
      real*8 uarg   ! Either u or u-u0 depending on model choice
      real*8 cosuarg , sinuarg    ! Cos/Sin of uarg
     .,      cos2uarg, sin2uarg   ! Cos/Sin of 2*uarg
     .,      cos4uarg, sin4uarg   ! Cos/Sin of 4*uarg
...
* MOD TAH 190702: Moved to arc.h and returned by svnav_read when 
*     igs_metadata.snx used.
C     real*8 antpwr
...
* MOD TAH 190709: Based on radiation model, set the arguments to be used: u or u-u0
      if( modrad.eq.1 ) then
         uarg = u 
      elseif( modrad.eq.2 ) then
         uarg = u - u0
         if( uarg.lt.0.d0 ) uarg = uarg + twopi
      endif
*     Compute trig function value
      cosuarg = dcos(uarg)
      sinuarg = dsin(uarg)
      cos2uarg = dcos(2*(uarg))
      sin2uarg = dsin(2*(uarg))
      cos4uarg = dcos(4*(uarg))
      sin4uarg = dsin(4*(uarg))
* Use these arguments below.  TAH 190709.
...
C         Check the output matches the Bar-Sever 1996 J Geod. output
     ...
     .       ,yatt_nom*360/twopi,beta*360/twopi,(uarg*360/twopi)-180
...
c             ratio is 0.64, the scaling is 0.64*0.8 = 0.5 
     ...
     .        antbody(1:9).eq.'BEIDOU-2G' .or.
     .        antbody(1:9).eq.'BEIDOU-3I' ) then 
...
c         For now, apply shadowing only to the direct constant term
     ...
     .                  ) * distfct
* MOD TAH 190615: Replaced the u argument with uarg so that BERNE and ECOMC
*         once per rev terms have the same phase (i.e., cosu -> cosuarg etc.
          radprs(i) = radprs(i) +
     .                   ( radcon(4) * d0 * cosuarg * snvec(i)
     .                   + radcon(5) * d0 * sinuarg * snvec(i)
     .                   + radcon(6) * d0 * cosuarg * yvec(i)
     .                   + radcon(7) * d0 * sinuarg * yvec(i)
     .                   + radcon(8) * d0 * cosuarg * bvec(i)
     .                   + radcon(9) * d0 * sinuarg * bvec(i) )*distfct 
         enddo                 
         if(debugsrp) 
...
c         ECOM2 uses arg from solar node
     ...
     .          +  radcon(7) * d0 * sinuarg * yvec(i)
     .          +  radcon(8) * d0 * cosuarg * bvec(i)
     .          +  radcon(9) * d0 * sinuarg * bvec(i) 
     .          +  radcon(10)* d0 * cos2uarg * snvec(i)
     .          +  radcon(11)* d0 * sin2uarg * snvec(i)
     .          +  radcon(12)* d0 * cos4uarg * snvec(i)
     .          +  radcon(13)* d0 * sin4uarg * snvec(i) ) * distfct
         enddo  

         if(debugsrp) 
     .     print *,'modrad radcon cosua sinua snvec yvec bvec radprs',
     .              modrad,radcon,cosuarg,sinuarg,snvec,yvec,bvec,radprs
...
C      Antenna thrust
* MOD TAH 190702: Don't initialize antpwr.  It can now be read from 
*      igs_metadata.snx and is set to zero in svnav_read if an svnav.dat
*      file is read.           
C      antpwr = 0.d0
...
* MOD TAH 190702: Call antpwr_base to get value based on satellite type 
*          if we don't have a power value read from igs_metadata.snx
           if( antpwr.eq.0 ) then 
              call antpwr_base( antbody, newsv, antpwr )
           endif 
        else
...
C------------------------------------------------------------------
C Write some information to the arcout file if first integration step for this satellite.
      if (newsv) then
         call keplr(sbcor,asat,eccen,wper0,fomeg0,fincl,anomin)         
	 call pjdhms(fJD,iyr,idoy,imonth,iday,ihr,imin)
         call jd_to_decyrs(fJD+0.5,decyrs)
         write(iarh,*) " "
         write(iarh,'(4a)') "          DECYR       YYYY MM DD HR MN DOY"
     .  ," PRN SVN BLOCK            MASS      ANTPWR   BETA    U0      "
     .  ,"     A           Ecc         Inc         Lon_A_N       Ar_Per"
     .  ,"      Mean_An" 
         write(iarh,500) decyrs,iyr,imonth,iday,ihr,imin,idoy,iprn,ssvn,
     .   antbody,sbmass,antpwr,beta*360.d0/twopi,u0*360.d0/twopi,asat,
     .   eccen,fincl*360.d0/twopi,fomeg0*360.d0/twopi,
     .   wper0*360.d0/twopi,anomin*360.d0/twopi
500      format("#orbinfo",1x,f12.6,i5,i3,i3,i3,i3,i4,i4,i4,1x,a16,f10.4
     .   ,f7.2,f8.3,f9.3,f14.7,5(1x,f12.6)) 
      endif
...
c     .       sbmass,
     .       beta*360.d0/twopi,
     .       u*360.d0/twopi,u0*360.d0/twopi,uarg*360.d0/twopi,
...
c-----end of condition on choice of model group
      ...
      if(debug) then    
        ...
        print *,'r s x y z b beta ',fjd,iprn,antbody,rvec,snvec,xvec,
     .           yvec,zvec,bvec,beta
        print*, 'rvec',rvec
        print*,' beta',beta
        print*, 'radprs ',radprs
...
* MOD TAH 190615: replaced cosu -> cosuarg sinu -> sinuarg to be constistent
...
* MOD TAH 180915 replace u with uarg
C          fn = fn + cosu * d0 * snvec(kk)  * distfct   
           fn = fn + cosuarg * d0 * snvec(kk)  * distfct   
         endif 
         if(debugsrp) print *,' 1pr D distfct d0 cosuarg snvec'
     . ,distfct,d0,cosuarg,snvec(kk)
...
* MOD TAH 180915 replace u with uarg
          fn = fn + sinuarg * d0 * snvec(kk)  * distfct
        endif    
        if(debugsrp) print *,'1pr D distfct d0 sinuarg snvec'
     .    ,distfct,d0,sinuarg,snvec(kk)
...
* MOD TAH 180915 replace u with uarg
          fn = fn + cosuarg * d0 * yvec(kk)  * distfct   
      endif   
      if(debugsrp) print *,'1pr Y distfct d0 cosuarg yvec'
     .  ,distfct,d0,cosuarg,yvec(kk)
...
* MOD TAH 180915 replace u with uarg
          fn = fn + sinuarg * d0 * yvec(kk)  * distfct
        endif     
        if(debugsrp) print *,'1pr Y distfct d0 sinuarg yvec'
     .    ,distfct,d0,sinuarg,yvec(kk)
...
* MOD TAH 180915 replace u with uarg
          fn = fn + cosuarg * d0 * bvec(kk)  * distfct
        endif 
        if(debugsrp)  print *,'1pr B distfct d0 cosuarg yvec'
     .     ,distfct,d0,cosuarg,bvec(kk)
...
* MOD TAH 180915 replace u with uarg
          fn = fn + sinuarg * d0 * bvec(kk)  * distfct
        endif 
...
*     partials to be consistent with force model (all 4 terms)
     ...
     .      ,'SRP parameters 10-13 defined only for ECOM2',0)
        fn = fn + cos2uarg * d0 * snvec(kk) * distfct
      elseif (kkk.eq.17) then
        fn = fn + sin2uarg * d0 * snvec(kk) * distfct
      elseif (kkk.eq.18) then
        fn = fn + cos4uarg * d0 * snvec(kk) * distfct
      elseif (kkk.eq.19) then  
        fn = fn + sin4uarg * d0 * snvec(kk) * distfct
      endif
