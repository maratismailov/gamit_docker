...
c      one day greater than the conventional Modified Julian date. 
       ...
       implicit none
* MOD AZ 190305: include Header for shared variables "nbody_path"
*                All related "open"&"report_stat" actions are
*                adjusted in all subroutines contained in this file
*                from "nbodyf" to "nbody_path"
       include 'track_com.h'         
c  Calling arguments
...
c      nbodyf = 'nbody' 
      open(unit=lun,file=nbody_path,iostat=ioerr)
      if( ioerr.eq.0 ) then
        call report_stat('STATUS',prog_name,'lib/ephred'
     .   ,nbody_path,'Opened planetary ephemeris file ',0)
      else
        call report_stat('FATAL',prog_name,'lib/ephred'
     .   ,nbody_path,'Error opening planetary ephemeris file',ioerr)
      endif 
...
c  a PEP sequential or binary  ascii file by reading the first three characters
     ...
     .    'Error reading 1st 3 characters of ephemeris file ',char3
        call report_stat('FATAL',prog_name,'lib/ephred',nbody_path,
     .                  message,ioerr)
      endif
      close(lun)
...
c      nbodyf = 'nbody'
      if( etype.eq.'PEPa') then        
        open( unit=lun,file=nbody_path,form='formatted',status='old'   
     .      , iostat= ioerr)   
     ...
     .         ,'nbody','Error opening PEP ascii file',ioerr)           
      elseif( etype.eq.'PEPb') then
        open( unit=lun,file=nbody_path,form='unformatted',status='old'   
     .      , iostat= ioerr)   
        if( ioerr.ne.0 ) call report_stat('FATAL',prog_name,'lib/ephred'
...
c       Earth 
     ...
     .         ,'lib/ephred','nbody','# Earth values > 15 ',0)
        ...
        call pleph( nbody_path,lun,et,ntarg,ncent,rrd ) 
        do i=1,6   
          earth(i,nsun) = rrd(i)
        enddo
        if( lbody.gt.0 ) then
...
C  THE SUBROUTINE ALSO SETS THE VALUES OF  NRECL, NRFILE, AND NAMFIL.
      ...
      INTEGER*4 IPT(3,13),nrecl,ksize,nrfile,mrecl,ncon
