...
C MOD TAH 210428: Modified to pass the center names through the 
C     runstring.  Increased max_center.  (Repro3 has 10).
C     Allows the order of the centers to be set.  All centers
C     found will be output.
 
      integer*4 max_site, max_center
 
      parameter ( max_site = 1000 )
      parameter ( max_center = 20 )
...
*   com_stats(4,3,max_center) -- Stats on stations common to COD,JPL,
*      SIO, GFZ, and MIT

 
      real*8 dpos(3), spos(3), pdiff(3,max_site, max_center),
     .    sdiff(3,max_site, max_center), site_stats(4,3,max_site),
     .    center_stats(4,3,max_center), com_stats(4,3,max_center)
...
* MOD TAH 210428: Added centers (passed as COD:EMR:JPL:MIT ...)
      character*120 centers

 
*   type(3)     - Position type (NEU)
*   av5         - Set to * if all 5 main centers are available
 
      character*1 type(3), av5
 
      data type  / 'N', 'E', 'U' /
 
***** Set up the center names
      num_center = 9
* MOD TAH 200720: Replaced COF with COD.
      center_names(1) = 'COD ' 
      center_names(2) = 'SIO ' 
      center_names(3) = 'GFZ ' 
      center_names(4) = 'MIT '
...
****  Get the runstring
      ...
      if( len_run.eq.0 ) then
          call proper_runstring('make_rms.hlp', 'make_rms', 1)
      end if
 
      open(100,file=infile, status='old', iostat=ierr)
      call report_error('IOSTAT',ierr,'open',infile,1,'make_rms')
...
*     Get the output file name
      ...
      call report_error('IOSTAT',ierr,'creat',outfile,1,'make_rms')
...
*     Get the start and end dates
      dow = GPS_week-int(GPS_week/10)*10
      GPS_week = int(GPS_week/10)
      call gpsw_date(gps_week, dow, date_start)

*     Write header to output file
      write(200,100,iostat=ierr) gps_week, dow, date_start 
100   format(/,80('*'),/,
     .       'MIT RESIDUAL REPORT GPS-WEEK ',i4.4,' DOW ',i1,14x,
     .       'Date: ',a8,/,80('*'))
...
****  Now see if centers have been passed
      len_run = rcpar(5,centers)
      if( len_run.gt.0 ) then
         num_center = (len_run+1)/4   ! +1 incase : not at end of line
         do i = 1,num_center
            center_names(i) = centers((i-1)*4+1:i*4-1)
         end do
         write(*,110) num_center, center_names(1:num_center)
 110     format('User: ',i2,' centers ',20(a3,1x))
      endif    
...
*             adjustments
              read(line,*,iostat=jerr) name, (dpos(i),spos(i),i=1,3),
     .                center
...
*     Now get the site common to all centers
      do i = 1, num_center
          do j = 1,3
              call get_stats( com_stats(1,j,i), pdiff, sdiff,
     .                max_site, max_center, num_site, num_center,
     .                i, j, 'COM')
          end do
      end do
...
* MOD TAH 060810: See if site is available at first five centers
          av5 = '*'
          do k = 1,5
             if( pdiff(1,j,k).eq.999.9d0 ) av5 = ' '
          enddo 
          do k = 1,3
     ...
     .            l = 1, num_center)
* MOD TAH 210429: Make sure repeat is large enough (20==max_centers)
400           format(1x,a4,1x,a1,i2,2F6.1,20(1x,F6.1,1x,F6.1,1x))
              call sub_char(line,'999.9','   -- ')
              line = line(1:trimlen(line)) // ' ' // av5
...
*     Write center statistics
 ...
 500  format(/'CENTER SUMMARY STATISTICS')
      write(200,510)
 510  format(25x,'All Sites',23x,'| Sites common between Centers')

      write(200,520) 
 520  format(' CENTER   #       North           East           Up',5x,
     .       ' | NC       North           East           Up',/,
     ...
     .         nint(com_stats(1,1,i)),
     .        (com_stats(3,k,i), com_stats(4,k,i), k = 1,3)
* MOD TAH 200417: Changed F6.1 to F6.2 format.  Also change sum.awk in
*     globals/sinex to match
 540      format(1x,a4,3x,i3,3(1x,F6.2,1x,F6.2,1x),
     .                 1x,'|',1x,i3,3(1x,F6.2,1x,F6.2,1x) )
      end do
...
*   num             - number of things to loop over
      ...
      integer*4 i, num_site, num_center, ent, cmp, num, j
...
*   CentAvail(5)  -- Logical set true if first 5 centers are
*     are available
      logical CentAvail(5)
...
****  See which of 5-centers are available.  If there are some
*     sites for the center then it is marked as available.
      if( type(1:2).eq.'CO' ) then
          do j = 1,5
             CentAvail(j) = .false.
             do i = 1,num_site
                 if( pdiff(cmp,i,j).ne.999.9d0 ) then
                     CentAvail(j) = .true.
                 endif
             end do
          end do
...
*             If we are doing only common sites see if this
*             site to common to COD, JPL, SIO, GFZ, MIT (These
*             are centers 5 and less
              if( type(1:2).eq.'CO' ) then
                 do j = 1,5
                    if ( pdiff(cmp,i,j).eq.999.9d0 .and. 
     .                   CentAvail(j) ) then
                       dpos = 999.9d0
                    endif
                 end do 
...
*   gps_start_mjd - MJD of the start of the GPS week count (80/1/5)
     ...
     .              'Jul','Aug','Sep','Oct','Nov','Dec'  /
      ...
      if( date(1).gt.100 ) date(1) = date(1) - 100
