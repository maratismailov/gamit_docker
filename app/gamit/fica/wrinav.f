      subroutine wrinav ( lu,site,nprn,iewkn,sow,bc_ephem,bc_clock
     .                  , subfr1 )
c
c     Write a RINEX navigation file from input FICA data
c     R. W. King  February 1990  - from MAKEX routine WORBIT

      integer*4 lu,nprn,iewkn,iyear,iy2,imonth,iday,idoy,ihr,min
     .        , itflag,i
      character*4 site
...
c        Convert GPS week, sec-of-week to calender day, hr, min, sec
c        (still GPS time, not UTC)

      itflag = +4
      call timcon ( itflag,iewkn,sow,iyear,idoy,ihr,min,sec,utcoff )
...
c       Write the epoch line with SV clock values
      ...
      iy2 = mod(iyear,100)
      write(lu,10) nprn,iy2,imonth,iday,ihr,min,sec
     .            , xeaf0,xeaf1,xeaf2
