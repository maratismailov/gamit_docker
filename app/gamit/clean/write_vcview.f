      subroutine write_vcview
     . (namec0,namev,imode,isite,imsite,lumf,versn,jstat)

c     Write a new V-file containing the edits of the current session
...
C MOD TAH 200126: Changed to svantdx(3,2,maxsat) (added L1/L2 index)
      ...
      real*8 rextra(maxext),save(maxsav),sec,sod
      real*8 obsv(maxdat),omcs(maxdat),elev,tmpart(maxlab)
      real*8 azim,svcepc,svcl1
      ...
      character*20 rlabel(maxlab),svantbody(maxsat)
      character*80 text(maxtxt)
      character*16 namev,namec0,uname
      character*80 versn
...
c     variables for new c-file format oct. 90, nov 91, apr 92, may 95
c     integer*4 okmet
      integer*4 okmet
      integer*4      avlmet
      integer*4    jde,jdr
     ...
     .,            speopmod,cextra(maxext) 
      ...
      integer*4 coord_index,atm_index,clock_index,orb_index(maxsat)
     .,svant_index(maxsat),eop_index,grad_index(2)
      real*8 grad_parts(2)
...
c     open a V-file to record cycle slips
     ...
     .      err    = 100)
      ...
      if (ioerr .ne. 0) then
         write (6,*) 'WRITE_VCVIEW: Could not open v-file-using scratch'
  ...
  100 continue
      if (ioerr .ne. 0) then
         call report_stat('WARNING','CVIEW','write_vcview',namev,
     .   'Error, opening V-file, continuing',ioerr)
c         stop 'in WRITE_VCVIEW: '  Do not stop but continue
      endif
...
c  tell the user which cfile we are reading
      write(6,544) namec0(1:nblen(namec0))
  544 format(' writing changes made to input c-file: ',a,
     .       ' to vcview.out',$)

c     read the header block 1 from input cfile (unit li)
...
c     get mapping from parameter slots to C-file partial slots
         call pointers( imsite,coord_index,atm_index,clock_index
...
c        seconds past initial epoch
  ...
  116       format ('Epoch counter mismatch.',2(i5,1x))
            call report_stat('STATUS','CVIEW','write_vcview',' '
     .     ,message,0)
         else
            if (kdoy .gt. 0) then
...
c           which is the correct PRN (sat id #)?
 ...
 16         continue
            ...
            call report_stat('FATAL','CVIEW','write_vcview',' '
     .                      ,message,0)
 17         continue
c
...
c               domc=addadj( is,isite,dt,iepoch,tpart )
     ...
     .                    , coord_index,atm_index,clock_index
     .                    , orb_index(is),svant_index,eop_index
     .                    , grad_index,grad_parts )
            else
               domc = 0.0d0
            endif
