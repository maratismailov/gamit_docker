...
C
C PARAMETERS :
C         IN :  ELEVATION : Elevetion angle of the Satellite     R*8
...
C                                                       
C CREATED    :  96/09/27              LAST MODIFIED :  96/10/18
C
...
C
C     N/S partial
      partial(1) = cos(azimuth)/(sin(elevation)*tan(elevation)+0.003)           
C
C     E/W partial
      partial(2) = sin(azimuth)/(sin(elevation)*tan(elevation)+0.003)           
C
      return
      end
