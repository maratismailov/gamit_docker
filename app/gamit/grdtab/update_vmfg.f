...
c      Program to create a VMF1 grid file or update it with  additional data 
c      (and eventually to update the header for new versions. 
c                                                             
c      R. King  August 2007 from update_atmlg; last updated 30 January 2008


c  Command line:  update_vmfg [input-file] [add-file] [output-file] [yr_start] [doy_start] [yr_end] [doy_end]
...
c                     if iadd-file] is a number, interpret as a [yr][doy], and add all ascii 
c                        files of the form all [ah/aw/zh][yr][doy].h[HH], nominally 12 (3 types x 4 epochs)
...
c
c     Examples:  update_vmfg vmf1grd.2007_155 vmf1grd.2007_154_160 vmf1grd.2007_160 
c                update_vmfg vmf1grd.2007_155 07156 vmf1grd.2007_156 
c    
c                                 
...
c  Maximum grid and file sizes
      ...
      parameter(maxlat=91,maxlon=145,maxfil=12)
        
c  Variables from the VMF files      
...
c   define the record length. For now, all VMF1 files have a record length of 12
      recl=12
...
c  Read the command line
      ...
      if(file_in(1:1).eq." ") then
        write(*,'(//,2a,//,2a,//,2a,/,a)')      
     .  "Run:  update_vmfg [in-file] [add-file] [out-file] [start yr] "
     .  ,       "[start doy] [end yr] [end doy]" 
     .  ,     "where [add-file] can be a binary file name or the date "
     .  ,       "(YYDDD) of Vienna ascii files (e.g., aw07156.h00)"  
     ...
     .  ,"If the dates are omitted, the dates on the files will be used"
      ...
      elseif ( file_add(1:3).eq.'vmf'.or.file_add(1:3).eq.'VMF') then
         ...
         call fix_y2k(yr4)
         start_yr_add = yr4
         read(file_add(3:5),'(f3.0)',iostat=ioerr) start_doy_add
