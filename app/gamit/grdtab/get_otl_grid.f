...
c     Interpolate a binary grid file of complex values amplitude and phase 
c     written by daf2da from Scherneck's daf files for pre-2012 models.  
c     Coded from old utils/grid_interp.f.    R. King  August 2006
c     Mods to read a version 2 grid file of U E N values written from ascii
c     versions of a netCDF file created for post-2012 grid files. R. King Mar 2018
c                              
c     File values are assumed to be complex radial displacements and 
c     complex tangential potential for each constituent.  

c     OSO grids have  11 primary constituents in the order
c     M2   S2   N2   K2   K1   O1   P1   Q1   Mf   Mm   Ssa   
c     FES2012 and later also have secondary waves (12-35) in the order
c     J1   L2   M4   M6   M8   N4   S1   S4   T2   2N2  J1+ 
c     K1-  K1+  K2+  LA2  M2-  MF+  MSF  MTM  MU2  N2-  O1-
c     Q1- SSA
c     The older 'daf' grids have 4 complex values for each wave.
c     The newer NetCDF grids have sin/cos for U E N, 6 values per wave.
                     
c     NAO grids have 21 constituents (84 complex) in the order   
c     M2   S2   K1   O1   N2   P1   K2   Q1   M1   J1   OO1  
c     2N2  Mu2  Nu2  L2   T2   Mtm  Mf   Mm   SSsa Sa 
c     For NAO the minor constituents are evaluated using the admittance, 
c     giving a total of 54, in the order
c     M2   S2   K1   O1   N2   P1   K2   Q1   M1   J1   OO1 
...
c     The grids are arranged in 360 latitude rings at 0.5-deg spacing from 90S 
c     to 89.5N or 90N to 89.5S, and 720 longitude values within each ring from 
c     0.5E to 360E.   In version 1, the values are real*4; in version 2 integer*2.
      
c     By convention and consistency with the OSO tide generator and OSO/GAMIT
c     list files, the output values in model.h common/ufcom/otides(maxotl,6) 
c     are amplitude and phase in Up, West, and South. 
...
c                      the grid.  For OSO, =11 or 36; for NAO, 21 are read from 
c                      the grid but admittance is used to compute 33 minor  
...
c     The third index (values) will be 4 (completx radius and ptan) for version 1
c     but 6 (U N E sin/cos) for version 2
      integer*2 nbox(2,2,6,maxotl)
      real*4    box(2,2,6,maxotl)           
      real*8 ampmj (16), phsmj (16)
      real*8 ampmn1(17), phsmn1(17)
      real*8 ampmn2(16), phsmn2(16)  
                              
      integer*4 ioerr,nrecs,irec,i,j,k,l,m
c      possibly temporary:
      integer*4 irec2 
      real*4 d,dinv,slat1,slon1,slat2,slon2,dlat,dlon,z1,z2,slon1t
     .     , usin,ucos,wsin,wcos,ssin,scos 
      character*256 message     
      logical first_call,debug/.false./
...
c       total number of records (for bounds checking)
        ...
        if(debug)  print *,'GET_OTL_GRID otlglon otlglat nrecs notlg '
     .    ,otlglon,otlglat,nrecs,notlg
...
c         loose tolerance is because #lat rings may be 361  
          if(debug) print *,'dat dlon ',dlat,dlon
          call report_stat('FATAL','GRDTAB','get_otl_grid','otl.grid' 
     .       ,'Lat and lon spacing not equal: something wrong',0)
        else
          d = dlon 
          dinv = 1./d
        endif
...
c     if longitude grid point is zero, use value for 360. (end of lat ring), but
c     still use the original value in the interpolation
      if( slon1.eq.0. ) then
        slon1t = 360.    
      else 
        slon1t = slon1
      ...
      slat2 = slat1 + d
c   lower left 
      irecbox(1,1) = grdpos(otlgns,otlglon,otlglat,d,slon1t,slat1) 
c   upper left
      irecbox(1,2) = grdpos(otlgns,otlglon,otlglat,d,slon1t,slat2)   
c   lower right
      irecbox(2,1) = grdpos(otlgns,otlglon,otlglat,d,slon2,slat1)
c   upper right
      irecbox(2,2) = grdpos(otlgns,otlglon,otlglat,d,slon2,slat2)
      if(debug) print *,'  otlgns slon1 slon2 slat1 slat2 irecbox '
     .      ,otlgns,slon1,slon2,slat1,slat2,((irecbox(i,j),i=1,2),j=1,2)
...
c         add one to account for the first (header) record
          irec = irecbox(i,j) + 1 
          if(debug) print *,' requesting irec ',irec
          if( irec.le.0 ) call report_stat('FATAL','GRDTAB'
     .         ,'get_otl_grid',' ','Requested grid file record < 0',0)
          if( irec.gt.nrecs ) then
             write(message,'(a,2f6.0,a,i6,a)') 'Requested record '
     .                        ,irec,'for lat,lon ',slat,slon
     .                     ,' exceeds records on grid file (',nrecs,')'
             call report_stat('FATAL','GRDTAB','get_otl_grid',' '
     .                    ,message,0)
          endif          
          if( otlgver.eq.1.0 ) then                                                
c           read real*4 complex rad, complex ptan for notlg constituents
            read(luotlg,rec=irec,iostat=ioerr) 
     .                             ((box(i,j,k,l),k=1,4),l=1,notlg)
            if( ioerr.ne.0 ) 
     .        call report_stat('FATAL','GRDTAB','get_otl_grid',' '
     .                    ,'Error reading Ver 1 grid file',ioerr)
          elseif( otlgver.eq.2.0 ) then
c  NOTE rwk 180313: For reasons I cannot discern, the values are off by 
c                   one tabular point in longitude for version 2. So
c                   for now, at least, increment the record by 1
              irec2 = irec + 1
c           read integer*2 rad west north sin and cos for notlg constituents
            read(luotlg,rec=irec2,iostat=ioerr) 
     .                             ((nbox(i,j,k,l),k=1,6),l=1,notlg)
            do l=1,notlg
              do k=1,6
                 box(i,j,k,l) = float(nbox(i,j,k,l))*otlgunits
              enddo
            enddo 
          endif 
        enddo
...
*     If file is byte-swapped, then switch the values just read
      if( otlgswap ) then
          call swap_bytes(4,box,2*2*6*notlg)  
      end if
                  
c     Version 1: interpolate to get the complex values
...
c           interpolate the complex values  
            print *,'slon slat slon1 slat1 dlon dlat '
     .              ,slon,slat,slon1,slat1,dlon,dlat
            print *,'box 11 21 22 12 ',
     .      box(1,1,1,m),box(2,1,1,m),box(2,2,1,m),box(1,2,1,m)
          endif
          z1 = terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,1,m)) 
          if(debug) print *,'z1 box ',z1,((box(i,j,1,1),i=1,2),j=1,2)
          z2 = terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,2,m)) 
          if(debug) print *,'z2 box ',z2,((box(i,j,2,1),i=1,2),j=1,2)
c         amplitude       
          otides(m,1) = sqrt(z1**2+z2**2)
c         phase
          otides(m,4) = atan2(z2,z1)/convd  
          if(debug) print *,'amp phase ',otides(m,1),otides(m,4)
        enddo 
cd      if(debug) stop 1
...
c         box(1,2,3,m) = 5.
          ...
          if(debug) print *,'z1 box ',z1,((box(i,j,3,1),i=1,2),j=1,2)
          z2 = -gradx2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,4,m))   
          if(debug) print *,'  z2 box ',z2,((box(i,j,4,1),i=1,2),j=1,2)
c         amplitude  
...
c         phase     
          ...
          if(debug) print *,'amp phase ',otides(m,2),otides(m,5)
c         south         
...
c         phase
          ...
          if(debug) print *,'amp phase ',otides(m,3),otides(m,6)
        enddo
        if(debug) print *,'slat slon slat1 slon1 ',slat,slon,slat1,slon1

c       Trap a problem with the CSR grids at the South Pole
                          
        if(debug) print *,'otidemod slat ',otidemod,slat
        if( otlgmod(1:3).eq.'CSR' .and. slat.lt.-89.5 ) then
          call report_stat('WARNING','GRDTAB','get_otl_grid',' '
     .      ,'CSR grids in error at South Pole, set components = 0.',0)
          do m=1,notlg
            do j=1,6
              otides(m,j) = 0.d0
            enddo
...
c       NAO has a different order and includes the secondary waves
c       so modify otides 
        if( otlgsrc.eq.'NAO' ) call otlnao

      elseif( otlgver.eq.2.0 ) then
        
c       Version 2: interpolate to get Up South West amplitude and phase from 
c                  gridded Up North East sine and cosine
        
        do m=1,notlg         
          if(debug) print *,otlwaves(m)
          if(debug) print *,' UP'
c         interpolate the sin and cos values  
          if(debug) print *,'slon slat slon1 slat1 dlon dlat '
     .            ,slon,slat,slon1,slat1,dlon,dlat
          if(debug) print *,'boxes 11 21 22 12 '
          usin = terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,1,m)) 
          if(debug) print *,'box usin',((box(i,j,1,1),i=1,2),j=1,2),usin
          ucos = terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,2,m)) 
          if(debug) print *,'box ucos',((box(i,j,2,1),i=1,2),j=1,2),ucos
c         amplitude       
          otides(m,1) = sqrt(usin**2+ucos**2)
c         phase  : Should this be atan2(ucos/usin) as for version 1? 
          otides(m,4) = atan2(usin,ucos)/convd  
          if(debug) print *,'amp phase ',otides(m,1),otides(m,4)
...
c         interpolate the sin and cos values  
          if(debug)  print *,'slon slat slon1 slat1 dlon dlat '
     .            ,slon,slat,slon1,slat1,dlon,dlat
          if(debug)  print *,'box 11 21 22 12 '
          wsin = -terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,3,m)) 
          if(debug) print *,'box wsin',((box(i,j,3,1),i=1,2),j=1,2),wsin
          wcos = -terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,4,m)) 
          if(debug) print *,'box wcos',((box(i,j,4,1),i=1,2),j=1,2),wcos
c         amplitude       
          otides(m,2) = sqrt(wsin**2+wcos**2)
c         phase  : Should this be atan2(cos/sin) as for version 1? 
          otides(m,5) = atan2(wsin,wcos)/convd  
          if(debug) print *,'amp phase ',otides(m,2),otides(m,5)   
     ...
     .          ,slon,slat,slon1,slat1,dlon,dlat
          if(debug)  print *,'box 11 21 22 12 '
          ssin = -terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,5,m)) 
          if(debug) print *,'box ssin',((box(i,j,5,1),i=1,2),j=1,2),ssin
          scos = -terp2(slon,slat,slon1,slat1,dlon,dlat,box(1,1,6,m)) 
          if(debug) print *,'box scos',((box(i,j,6,1),i=1,2),j=1,2),scos
c         amplitude       
          otides(m,3) = sqrt(ssin**2+scos**2)
c         phase  : Should this be atan2(cos/sin) as for version 1? 
          otides(m,6) = atan2(ssin,scos)/convd  
          if(debug) print *,'amp phase ',otides(m,3),otides(m,6)   
        enddo      
      endif

      write(luprnt,'(a,a8,1x,54(1x,a3))') 
     .      '  OTL  grid ',otlgmod,(otlwaves(i),i=1,notlg)
...
c     Code from get_otl_grid to change the order of the primary waves
c     and add the secondary waves for the NAO version 1  grid
...
c ------
c     n = # lat values, not used in formulas, but keep for now and
c     add this dummy statement to avoid a compiler warning:
      if( n.eq.0 ) then
         write(*,*) 'DUMMY n ', n
      endif
c -----
...
cd     print *,'TERP2 11 21 22 12 ',za(1,1),za(2,1),za(2,2),za(1,2)
       terp2 =  (1.-t)*(1.-u)*za(1,1) + t*(1.-u)*za(2,1)
