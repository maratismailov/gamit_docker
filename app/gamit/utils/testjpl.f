...
C      Print the ephemeris constants.
       ...
       CALL  Const (namfil, nrfile, NAMS, VALS, SS, NVS)
...
c        be near zero for parituclar components.)
...
c     by scaling DEL down for psi and d(psi)/dt, as opposed to having differnt
c     tolerances for different types of quantities.
...
C ************************************************************************
     ...
     *       RECL=IRECSZ,
     *       STATUS='OLD')
      print *,'opened in STATE',nrfile,namfil,irecsz,swap
     ...
     . ((IPT(I,J),I=1,3),J=1,12),NUMDE,(IPT(I,13),I=1,3)    
      ...
      if( swap ) then   
        ...
        do i=1,3
           call swap_bytes(8,ss,3)  
        enddo
        print *,'swapped ncon,numde,ss,au,emrat',ncon,numde,ss,au,emrat
        do j=1,13   
          do i=1,3
            call swap_bytes(4,ipt(i,j),1)
          enddo
...
C+++++++++++++++++++++++++++++
C
      SUBROUTINE CONST(namfil,nrfile,NAM,VAL,SSS,N)
C
C+++++++++++++++++++++++++++++
C
...
C  CALL STATE TO INITIALIZE THE EPHEMERIS AND READ IN THE CONSTANTS
      ...
      IF(FIRST) CALL STATE(namfil,nrfile,zips,list,xx,xx)
      first=.false.
