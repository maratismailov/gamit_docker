...
c      IMPLICIT DOUBLE PRECISION (A-H,O-Z),LOGICAL(L)
      ...
      real*8 xi,yi,zi,xdoti,ydoti,zdoti,gm
      real*8 a,e,rnode,omega,rmean,x,y,z,xdot,ydot,zdot
      real*8 twopi,two,degrad,scale,one,rrdot,r,vsq
      real*8 ainv,hx,hy,hz,hsini2,hsq,h,ome2,esq,tempe
      real*8 cosi,sini2,sini,hsini,zero,suprod
      real*8 cuprod,resinf,recosf,true,surecf,curecf,curesf
      real*8 ccc,swprod,cwprod,rtome2,aecose,ecc,halfi,sinhaf
      real*8 coshaf,sinn,cosn,somega,comega,coson,sinon
      real*8 c1,ca1,ca2,ch1,ch2,ch3,eprod,ce2,cn1,cn2,cu5
      real*8 cosu,hcosu,hzcosu,sinu,hsinu,cu1,cu2,cu3,cu4,adr
      real*8 rincl,suresf,aesine,csprod,ce1,re,cosf,sinf
      real*8 cf1,cf2,cf3,cf4,cf5,rda,cm1,cm2,ppfs,ppus
      real*8 pfps
      real*8 phps,pups,scale2,c2j,c3j,c4j,c5j,c6j,c2

      integer*4 idrad,ihypsw,ieven,iparts,jflip,i,j
      logical lparts,lhyper,lneven,lnons,lkep5

      real*8 AEI(6),AEINPM(6),PKPX(6,6),XYZXYZ(6),
     &PH(3,6),CUXYZP(3)
*
...
*  COMPUTE PARTIALS OF NONSINGULAR KEPLER ELEMENTS W/R TO CARTESIAN ELEM
  ...
  333 CONTINUE
      RETURN
 500  call report_stat('FATAL','SOLVE','elem',' '
     .          , 'S/C coordinates imply hyperbolic trajectory',0)
      END
