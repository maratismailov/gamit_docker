...
C   DSPSL  solves the double precision symmetric system
C          a * x = b
C          using the factors computed by DSPFA.
C   DSVDC  is a subroutine to reduce a double precision nxp matrix x
C          by orthogonal transformations u and v to diagonal form.
...
c modifications:  a(1) changed to a(*) for sun compiler Kurt Feigl Aug. 1. 1990
c                 lines commented out with ckurt
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             CHOLESKY,POSITIVE DEFINITE
C***DATE WRITTEN  MARCH 19, 1979
C***AUTHOR  DONGARRA J. (ANL), G. STEWART (U MD)
C***PURPOSE
C   COMPUTES THE CHOLESKY DECOMPOSITION OF A POSITIVE DEFINITE
C     MATRIX.  A PIVOTING OPTION ALLOWS THE USER TO ESTIMATE THE
C     CONDITION OF A POSITIVE DEFINITE MATRIX OR DETERMINE THE RANK
C     OF A POSITIVE SEMIDEFINITE MATRIX.
C***DESCRIPTION
...
ckurt  INTEGER LDA,P,JPVT(1),JOB,INFO
      INTEGER LDA,P,JPVT(*),JOB,INFO
ckurt DOUBLE PRECISION A(LDA,1),WORK(1)
...
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C DOWNDATES AN AUGMENTED CHOLESKY DECOMPOSITION OR THE TRIANGULAR
C   FACTOR OF AN AUGMENTED QR DECOMPOSITION.
C***DESCRIPTION
C     DCHDD DOWNDATES AN AUGMENTED CHOLESKY DECOMPOSITION OR THE
C     TRIANGULAR FACTOR OF AN AUGMENTED QR DECOMPOSITION.
...
ckurt DOUBLE PRECISION R(LDR,1),X(1),Z(LDZ,1),Y(1),S(1)
ckurt DOUBLE PRECISION RHO(1),C(1)
...
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C  UPDATES THE CHOLESKY FACTORIZATION  A=TRANS(R)*R  OF A POSITIVE
C  DEFINITE MATRIX A OF ORDER P UNDER DIAGONAL PERMUTATIONS OF THE
C   FORM  TRANS(E)*A*E  WHERE E IS A PERMUTATION MATRIX.
C***DESCRIPTION
C     DCHEX UPDATES THE CHOLESKY FACTORIZATION
C
C                   A = TRANS(R)*R
...
ckurt DOUBLE PRECISION R(LDR,1),Z(LDZ,1),S(1)
ckurt DOUBLE PRECISION C(1)
...
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C  UPDATES AN AUGMENTED CHOLESKY DECOMPOSITION OF THE TRIANGULAR PART
C   OF AN AUGMENTED QR DECOMPOSITION.
C***DESCRIPTION
C     DCHUD UPDATES AN AUGMENTED CHOLESKY DECOMPOSITION OF THE
...
C***CATEGORY NO.  F4H
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             CONDITION NUMBER,BAND
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C   FACTORS A DOUBLE PRECISION BAND MATRIX BY GAUSSIAN
C     ELIMINATION AND ESTIMATES THE CONDITION OF THE MATRIX.
C***DESCRIPTION
C     DGBCO FACTORS A DOUBLE PRECISION BAND MATRIX BY GAUSSIAN
C     ELIMINATION AND ESTIMATES THE CONDITION OF THE MATRIX.
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DSCAL,DGBFA,DASUM,DDOT
C***END PROLOGUE  DGBCO
ckurt INTEGER LDA,N,ML,MU,IPVT(1)
      INTEGER LDA,N,ML,MU,IPVT(*)
ckur  DOUBLE PRECISION ABD(LDA,1),Z(1)
...
C***REVISION DATE  811015   (YYMMDD)
C***CATEGORY NO.  F4H, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             BAND,DETERMINABT
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C COMPUTES THE DETERMINANT OF A BAND MATRIX USING FACTORS COMPUTED BY
C  DGBCO OR DGBFA. USE DGBSL N TIMES FOR THE INVERSE.
C***DESCRIPTION
C     DGBDI COMPUTES THE DETERMINANT OF A BAND MATRIX
C     USING THE FACTORS COMPUTED BY DGBCO OR DGBFA.
C     IF THE INVERSE IS NEEDED, USE DGBSL  N  TIMES.
...
ckurt INTEGER LDA,N,ML,MU,IPVT(1)
ckurt DOUBLE PRECISION ABD(LDA,1),DET(2)
      INTEGER LDA,N,ML,MU,IPVT(*)
...
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C   FACTORS A DOUBLE PRECISION BAND MATRIX BY ELIMINATION.
C***DESCRIPTION
C     DGBFA FACTORS A DOUBLE PRECISION BAND MATRIX BY ELIMINATION.
C
...
ckurt INTEGER LDA,N,ML,MU,IPVT(1),INFO
ckurt DOUBLE PRECISION ABD(LDA,1)
...
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C SOLVES THE DOUBLE PRECISION BAND SYSTEM  A*X=B OR  TRANS(A)*X=B
C     USING THE FACTORS COMPUTED BY DGBCO OR DGBFA.
C***DESCRIPTION
C     DGBSL SOLVES THE DOUBLE PRECISION BAND SYSTEM
C     A * X = B  OR  TRANS(A) * X = B
...
ckurt INTEGER LDA,N,ML,MU,IPVT(1),JOB
ckurt DOUBLE PRECISION ABD(LDA,1),B(1)
...
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C   FACTORS A DOUBLE PRECISION MATRIX BY GAUSSIAN ELIMINATION
C     AND ESTIMATES THE CONDITION OF THE MATRIX.
C***DESCRIPTION
C     DGECO FACTORS A DOUBLE PRECISION MATRIX BY GAUSSIAN ELIMINATION
C     AND ESTIMATES THE CONDITION OF THE MATRIX.
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DSCAL,DGEFA,DASUM,DDOT
C***END PROLOGUE  DGECO
ckurt INTEGER LDA,N,IPVT(1)
ckurt DOUBLE PRECISION A(LDA,1),Z(1)
      INTEGER LDA,N,IPVT(*)
...
C***REVISION DATE  811015   (YYMMDD)
C***CATEGORY NO.  F4A, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             DETERMINANT,INVERSE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C COMPUTES THE DETERMINANT AND INVERSE OF A MATRIX USING FACTORS
C   COMPUTED BY DGECO OR DGEFA.
C***DESCRIPTION
C     DGEDI COMPUTES THE DETERMINANT AND INVERSE OF A MATRIX
C     USING THE FACTORS COMPUTED BY DGECO OR DGEFA.
C
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DSWAP,DAXPY,DSCAL
C***END PROLOGUE  DGEDI
ckurt INTEGER LDA,N,IPVT(1),JOB
ckurt DOUBLE PRECISION A(LDA,1),DET(2),WORK(1)
...
ckurt INTEGER LDA,N,IPVT(1),INFO
ckurt DOUBLE PRECISION A(LDA,1)
...
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C  SOLVES THE DOUBLE PRECISION SYSTEM  A*X=B OR  TRANS(A)*X=B
C     USING THE FACTORS COMPUTED BY DGECO OR DGEFA.
C***DESCRIPTION
C     DGESL SOLVES THE DOUBLE PRECISION SYSTEM
C     A * X = B  OR  TRANS(A) * X = B
...
ckurt INTEGER LDA,N,IPVT(1),JOB
ckurt DOUBLE PRECISION A(LDA,1),B(1)
      INTEGER LDA,N,IPVT(*),JOB
...
C***PURPOSE
C SOLVES THE SYSTEM  T*X=B  WHERE T IS A GENERAL TRIDIAGONAL MATRIX.
C***DESCRIPTION
C     DGTSL GIVEN A GENERAL TRIDIAGONAL MATRIX AND A RIGHT HAND
C     SIDE WILL FIND THE SOLUTION.
C
...
C***END PROLOGUE  DGTSL
      INTEGER N,INFO
ckurt DOUBLE PRECISION C(1),D(1),E(1),B(1)
...
C***REVISION DATE  811015   (YYMMDD)
C***CATEGORY NO.  F4B, F4H
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,BAND,POSITIVE DEFINITE,CONDITION NUMBER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C FACTORS A D.P. SYMMETRIC POSITIVE DEFINITE MATRIX STORED IN
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DSCAL,DPBFA,DASUM,DDOT
C***END PROLOGUE  DPBCO
      INTEGER LDA,N,M,INFO
ckurt DOUBLE PRECISION ABD(LDA,1),Z(1)
      DOUBLE PRECISION ABD(LDA,*),Z(*)
      DOUBLE PRECISION RCOND
C
      DOUBLE PRECISION DDOT,EK,T,WK,WKM
...
C***REVISION DATE  811015   (YYMMDD)
C***CATEGORY NO.  F4B, F4H, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,BAND,DETERMINANT
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C COMPUTES THE DETERMINANT OF A D.P. SYMMETRIC POSITIVE DEFINITE BAND
C MATRIX USING FACTORS OF DPBCO OR DPBFA. USE DPBSL N TIMES FOR INVERSE.
C***DESCRIPTION
C     DPBDI COMPUTES THE DETERMINANT
C     OF A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE BAND MATRIX
C     USING THE FACTORS COMPUTED BY DPBCO OR DPBFA.
...
C***CATEGORY NO.  F4B, F4H
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             BAND,SYMMETRIC,POSITIVE DEFINITE,FACTOR
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C  FACTORS A D.P. SYMMETRIC POSITIVE DEFINITE MATRIX STORED IN BAND FORM
C***DESCRIPTION
C     DPBFA FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX STORED IN BAND FORM.
C
...
C***CATEGORY NO.  F4B, F4H
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             BAND,POSITIVE DEFINITE,SYMMETRIC,SOLVE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C SOLVES THE D.P. SYMMETRIC POSITIVE DEFINITE BAND SYSTEM A*X=B
C  USING THE FACTORS COMPUTED BY DPBCO OR DPBFA.
C***DESCRIPTION
C     DPBSL SOLVES THE DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     BAND SYSTEM  A*X = B
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,POSITIVE DEFINITE,CONDITION NUMBER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C  FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX AND ESTIMATES THE CONDITION OF THE MATRIX.
C***DESCRIPTION
C     DPOCO FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX AND ESTIMATES THE CONDITION OF THE MATRIX.
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DSCAL,DPOFA,DASUM,DDOT
...
C***REVISION DATE  811015   (YYMMDD)
C***CATEGORY NO.  F4B, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,POSITIVE DEFINITE,DETERMINANT,INVERSE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C  COMPUTES THE DETERMINANT AND INVERSE OF A CERTAIN DOUBLE
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,POSITIVE DEFINITE,FACTOR
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,POSITIVE DEFINITE,SOLVE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C SOLVES THE DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE SYSTEM A*X=B
C     USING THE FACTORS COMPUTED BY DPOCO OR DPOFA.
C***DESCRIPTION
C     DPOSL SOLVES THE DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     SYSTEM A * X = B
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,POSITIVE DEFINITE,CONDITION NUMBER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE MATRIX STORED
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DSCAL,DPPFA,DASUM,DDOT
C***END PROLOGUE  DPPCO
      INTEGER N,INFO
ckurt DOUBLE PRECISION AP(1),Z(1)
...
C***CATEGORY NO.  F4B, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,POSITIVE DEFINITE,DETERMINANT,
C             INVERSE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C  COMPUTES THE DETERMINANT AND INVERSE OF A D.P. SYMMETRIC POSITIVE
C DEFINITE MATRIX USING FACTORS FROM DPPCO OR DPPFA.
C***DESCRIPTION
C     DPPDI COMPUTES THE DETERMINANT AND INVERSE
C     OF A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE MATRIX
C     USING THE FACTORS COMPUTED BY DPPCO OR DPPFA .
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,POSITIVE DEFINITE,FACTOR
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C FACTORS A D.P. SYMMETRIC POSITIVE DEFINITE MATRIX (PACKED FORM)
C***DESCRIPTION
C     DPPFA FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX STORED IN PACKED FORM.
C
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,POSITIVE DEFINITE,SOLVE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C SOLVES THE DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE SYSTEM A*X=B
...
C***END PROLOGUE  DPPSL
      INTEGER N
ckurt DOUBLE PRECISION AP(1),B(1)
...
C***CATEGORY NO.  F4B
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             POSITIVE DEFINITE,TRIDIAGONAL,SOLVE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  DONGARRA J. (ANL)
C***PURPOSE
C SOLVES THE SYSTEM  T*X=B  WHERE T IS POSITIVE DEFINITE TRIDIAGONAL.
C***DESCRIPTION
C     DPTSL GIVEN A POSITIVE DEFINITE TRIDIAGONAL MATRIX AND A RIGHT
C     HAND SIDE WILL FIND THE SOLUTION.
C
...
C***END PROLOGUE  DPTSL
      INTEGER N
ckurt DOUBLE PRECISION D(1),E(1),B(1)
...
C***CATEGORY NO.  F6
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             QR DECOMPOSITION,HOUSEHOLDER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C   USES HOUSEHOLDER TRANSFORMATIONS TO COMPUTE THE QR FACTORIZATION OF
C   N BY P MATRIX X.  COLUMN PIVOTING IS OPTIONAL.
C***DESCRIPTION
C     DQRDC USES HOUSEHOLDER TRANSFORMATIONS TO COMPUTE THE QR
C     FACTORIZATION OF AN N BY P MATRIX X.  COLUMN PIVOTING
C     BASED ON THE 2-NORMS OF THE REDUCED COLUMNS MAY BE
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DSCAL,DSWAP,DDOT,DNRM2
C***END PROLOGUE  DQRDC
      INTEGER LDX,N,P,JOB
ckurt INTEGER JPVT(1)
ckurt DOUBLE PRECISION X(LDX,1),QRAUX(1),WORK(1)
      INTEGER JPVT(*)
...
C***CATEGORY NO.  F6
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             LEAST SQUARES,PROJECTION,COORDINATE TRANSFORMATION
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C  APPLIES THE OUTPUT OF DQRDC TO COMPUTE COORDINATE TRANSFORMATIONS,
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DAXPY,DDOT,DCOPY
C***END PROLOGUE  DQRSL
      INTEGER LDX,N,K,JOB,INFO
ckurt DOUBLE PRECISION X(LDX,1),QRAUX(1),Y(1),QY(1),QTY(1),B(1),RSD(1),
ckurt*                 XB(1)
...
C***CATEGORY NO.  F4C
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,CONDITION NUMBER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C FACTORS A D.P. SYMMETRIC MATRIX BY ELIMINATION WITH SYMMETRIC PIVOTING
C  AND ESTIMATES THE CONDITION OF THE MATRIX.
C***DESCRIPTION
C     DSICO FACTORS A DOUBLE PRECISION SYMMETRIC MATRIX BY ELIMINATION
C     WITH SYMMETRIC PIVOTING AND ESTIMATES THE CONDITION OF THE
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DSCAL,DAXPY,DSIFA,DASUM,DDOT
C***END PROLOGUE  DSICO
ckurt INTEGER LDA,N,KPVT(1)
ckurt DOUBLE PRECISION A(LDA,1),Z(1)
      INTEGER LDA,N,KPVT(*)
      DOUBLE PRECISION A(LDA,*),Z(*)
      DOUBLE PRECISION RCOND
...
C***REVISION DATE  811015   (YYMMDD)
C***CATEGORY NO.  F4C, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             SYMMETRIC,DETERMINANT,INVERSE,INERTIA
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  BUNCH J. (UCSD)
C***PURPOSE
C    COMPUTES THE DETERMINANT, INERTIA AND INVERSE OF A D.P.
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DSWAP,DAXPY,DCOPY,DDOT
C***END PROLOGUE  DSIDI
      INTEGER LDA,N,JOB
ckurt DOUBLE PRECISION A(LDA,1),WORK(1)
      DOUBLE PRECISION A(LDA,*),WORK(*)
      DOUBLE PRECISION DET(2)
ckurt INTEGER KPVT(1),INERT(3)
...
ckurt INTEGER LDA,N,KPVT(1),INFO
ckurt DOUBLE PRECISION A(LDA,1)
...
C***CATEGORY NO.  F4C
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,CONDITION NUMBER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C FACTORS A D.P. SYMMETRIC MATRIX STORED IN PACKED FORM BY ELIMINATION
C   WITH SYMMETRIC PIVOTING AND ESTIMATES THE CONDITION OF THE MATRIX.
C***DESCRIPTION
C     DSPCO FACTORS A DOUBLE PRECISION SYMMETRIC MATRIX STORED IN
C     PACKED FORM BY ELIMINATION WITH SYMMETRIC PIVOTING AND ESTIMATES
C     THE CONDITION OF THE MATRIX.
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED DSCAL,DAXPY,DSPFA,DASUM,DDOT
C***END PROLOGUE  DSPCO
ckurt INTEGER N,KPVT(1)
ckurt DOUBLE PRECISION AP(1),Z(1)
      INTEGER N,KPVT(*)
      DOUBLE PRECISION AP(*),Z(*)
      DOUBLE PRECISION RCOND
...
C***CATEGORY NO.  F4C, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,DETERMINANT,INVERSE,INERTIA
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  BUNCH J. (UCSD)
C***PURPOSE
C  COMPUTES THE DETERMINANT, INERTIA AND INVERSE OF A
...
C***END PROLOGUE  DSPDI
      INTEGER N,JOB
ckurt DOUBLE PRECISION AP(1),WORK(1)
...
C***CATEGORY NO.  F4C
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             PACKED,SYMMETRIC,SOLVE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  BUNCH J. (UCSD)
C***PURPOSE
C  SOLVES THE DOUBLE PRECISION SYMMETRIC SYSTEM  A*X=B
C     USING THE FACTORS COMPUTED BY DSPFA.
C***DESCRIPTION
C     DSISL SOLVES THE DOUBLE PRECISION SYMMETRIC SYSTEM
C     A * X = B
...
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C  PERFORM THE SINGULAR VALUE DECOMPOSITION OF A D.P. NXP MATRIX
C***DESCRIPTION
C     DSVDC IS A SUBROUTINE TO REDUCE A DOUBLE PRECISION NXP MATRIX X
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED  DSWAP,DROT,DROTG,DAXPY,DSCAL,DNRM2,DDOT
C***END PROLOGUE  DSVDC
      INTEGER LDX,N,P,LDU,LDV,JOB,INFO
ckurt DOUBLE PRECISION X(LDX,1),S(1),E(1),U(LDU,1),V(LDV,1),WORK(1)
...
C***CATEGORY NO.  F4A
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             TRIANGULAR,CONDITION NUMBER
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C ESTIMATES THE CONDITION OF A DOUBLE PRECISION TRIANGULAR MATRIX.
C***DESCRIPTION
C     DTRCO ESTIMATES THE CONDITION OF A DOUBLE PRECISION TRIANGULAR
C     MATRIX.
C
...
C  DONGARRA J.J., BUNCH J.R., MOLER C.B., STEWART G.W.,
C   *LINPACK USERS  GUIDE*, SIAM, 1979.
C***ROUTINES CALLED   DAXPY,DSCAL,DASUM
C***END PROLOGUE  DTRCO
      INTEGER LDT,N,JOB
ckurt DOUBLE PRECISION T(LDT,1),Z(1)
...
C***CATEGORY NO.  F4A, F3
C***KEYWORDS  LINPACK,MATRIX,LINEAR EQUATIONS,DOUBLE PRECISION,
C             TRIANGULAR,DETERMINANT,INVERSE
C***DATE WRITTEN  AUGUST 14, 1978
C***AUTHOR  MOLER C.B. (UNM)
C***PURPOSE
C COMPUTES THE DETERMINANT AND INVERSE OF A D.P. TRIANGULAR MATRIX.
C***DESCRIPTION
C     DTRDI COMPUTES THE DETERMINANT AND INVERSE OF A DOUBLE PRECISION
C     TRIANGULAR MATRIX.
C
...
C***AUTHOR  STEWART G.W. (U MARYLAND)
C***PURPOSE
C SOLVES SYSTEMS OF THE FORM  T*X=B OR  TRANS(T)*X=B  WHERE T
...
C***END PROLOGUE  DTRSL
      INTEGER LDT,N,JOB,INFO
ckurt DOUBLE PRECISION T(LDT,1),B(1)
