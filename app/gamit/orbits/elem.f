...
*                      SATELLITES)
*   IHYPSW    I   S    IF =1, THEN HYPERBOLIC IS ACCEPTED WITHOUT
*                      TERMINATION
*
*
* COMMENTS:
...
*********1*********2*********3*********4*********5*********6*********7**
      ...
      integer IDRAD,IHYPSW,IEVEN,IPARTS,I,J,JFLIP,len,rcpar
      real*8 zero,one,two,aei,pkpx,aeinpm,xyzxyz,ph,cuxyzp,a,e,rincl
     &     , rnode,omega,rmean,x,y,z,xdot,ydot,zdot,twopi,degrad,scale
     &     , xi,yi,zi,xdoti,ydoti,zdoti,rrdot,r,vsq,ainv,gm,hx,hy,hz
     &     , hsini2,hsq,h,ome2,esq,tempe,cosi,sini2,sini,hsini,suprod
     &     , cuprod,resinf,recosf,true,surecf,curecf,curesf,suresf,ccc
     &     , swprod,cwprod,rtome2,aesine,aecose,ecc,halfi,sinhaf,coshaf
     &     , sinn,cosn,csprod,somega,comega,coson,sinon,c1,ca1,ca2,ch1
     &     , ch2,ch3,eprod,ce1,ce2,cn1,cn2,cu5,cosu,hcosu,hzcosu,sinu
     &     , hsinu,cu1,cu2,cu3,cu4,adr,re,cosf,sinf,c2,cf1,cf2,cf3,cf4
     &     , cf5,rda,cm1,cm2,ppfs,ppus,phps,pups,pfps,scale2,c2j,c3j
     &     , c4j,c5j,c6j
      logical lneven,lnons,lkep5,lparts,lhyper
      SAVE
CCC      LOGICAL LHYPER,HYPSW,LNEVEN,LKEP5,LNONS,LPARTS
...
c     get calling program name name for report_stat
      len = rcpar(0,prog_name)

      TWOPI=DACOS(-1.D0)*TWO
      DEGRAD=TWOPI/360.D0

      ieven=mod(idrad,2)
      if(ieven.eq.0) then
        lneven = .true.
      elseif(ieven.eq.1) then
        lneven = .false.
      endif
c**      LNEVEN=MOD(IDRAD,2).EQ.0
      lnons=.false.
      if(idrad.ge.8) lnons=.true.
c**      LNONS=IDRAD.GE.8
      lkep5=.false.
      if(idrad.ge.4.and.idrad.le.8) lkep5=.true.
c**      LKEP5=IDRAD.GE.4.AND.IDRAD.LE.8
      iparts=mod(idrad,4)/2
      if(iparts.eq.0) then
        lparts=.false.
      elseif(iparts.eq.1) then
        lparts=.true.
      endif
c**      LPARTS=MOD(IDRAD,4)/2.EQ.1
c      write(*,*) lneven,lnons,lkep5,lparts
c*
      SCALE=ONE
...
*  COMPUTE NON-SINGULAR KEPLER ELEMENTS
      HALFI=0.5D0*RINCL
      SINHAF=SIN(HALFI)
      COSHAF=DSQRT(ONE-SINHAF*SINHAF)
      SINN=HX/HSINI
      COSN=-HY/HSINI
      CSPROD=DSQRT(CWPROD*CWPROD+SWPROD*SWPROD)
...
*  COMPUTE PARTIALS OF NONSINGULAR KEPLER ELEMENTS W/R TO CARTESIAN ELEM
  ...
  333 CONTINUE
      RETURN
 500  WRITE(message,1500)
1500  FORMAT('CARTESIAN SPACECRAFT COORDINATES EQUIVALENT ',
     &   'TO HYPERBOLIC TRAJECTORY. EXECUTION TERMINATING.')
      call report_stat('FATAL',prog_name,'orbits/elem',' ',message,0)
